export function buildLangChainWiringInstructions(projectDir: string, agentIdentifier: string): string {
  return [
    ...
    '',
    '1. Create app/api/novu/route.ts with serve({ agents: [...] }) from @notify/framework/next',
    `2. Create app/novu/agents/${agentIdentifier}.tsx with agent() from @notify/framework/langchain`,
