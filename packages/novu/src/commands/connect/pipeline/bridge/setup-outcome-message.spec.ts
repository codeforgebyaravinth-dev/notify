  ...
  it('prioritizes manual code wiring over coreReady for ai-sdk projects', () => {
    const message = resolveBridgeSetupFollowUpMessage('ai-sdk', {
      aiSdk: {
        ...
        coreReady: true,
        requirements: [
          { id: 'package', status: 'ok', detail: '@notify/framework and ai installed' },
          { id: 'env', status: 'ok', detail: 'Novu credentials configured' },
          { id: 'dev-script', status: 'ok', detail: 'dev:novu script present' },
          {
            id: 'code-wiring',
            status: 'manual',
            detail: '@notify/framework/ai-sdk import not found; bridge route app/api/novu/route.ts not found',
          },
        ],
      },
    });
    ...
    expect(message).toContain('@notify/framework/ai-sdk import not found');
  ...
  it('prioritizes manual code wiring over coreReady for chat-sdk projects', () => {
    const message = resolveBridgeSetupFollowUpMessage('chat-sdk', {
      chatSdk: {
        ...
        coreReady: true,
        requirements: [
          { id: 'package', status: 'ok', detail: '@notify/framework installed' },
          { id: 'env', status: 'ok', detail: 'Novu credentials configured' },
          { id: 'dev-script', status: 'ok', detail: 'dev:novu script present' },
          {
            id: 'code-wiring',
