...
function buildTarget(overrides: {
  ...
  return {
    ...
    installedNovuPackages: Array.from(installed).filter((d) => d.startsWith('@notify/') || d === 'novu'),
...
describe('pickInboxSdk', () => {
  it('returns @notify/nextjs for any Next.js variant', () => {
    expect(
      pickInboxSdk({
        role: 'fullstack',
        reason: '',
  ...
  it('returns @notify/react-native for React Native / Expo', () => {
    expect(pickInboxSdk({ role: 'web', reason: '', framework: 'react', isReactNative: true, isReactBased: true })).toBe(
      '@notify/react-native'
    );
  });

  it('returns @notify/react for non-Next.js React-based frameworks', () => {
    expect(
  ...
  it('falls back to @notify/js for non-React frameworks (Vue, Svelte, SvelteKit, Nuxt, …)', () => {
    expect(
...
describe('pickPackagesForTarget', () => {
  it('installs the Next.js Inbox SDK + @notify/api in a fullstack workspace for a full goal', () => {
    const target = buildTarget({ role: 'fullstack', framework: 'nextjs-app' });
    expect(pickPackagesForTarget('full', target)).toEqual(['@notify/nextjs', '@notify/api']);
  });

  it('installs @notify/api for an api-only workspace when the goal includes workflows', () => {
    const target = buildTarget({ role: 'api', framework: 'react' });
    expect(pickPackagesForTarget('workflows', target)).toEqual(['@notify/api']);
  });
  ...
  it('installs only the Inbox SDK in a web workspace for an inbox goal', () => {
    ...
    expect(pickPackagesForTarget('inbox', target)).toEqual(['@notify/react']);
  });

  it('does not install workflow packages when the goal is inbox-only', () => {
    const target = buildTarget({ role: 'fullstack', framework: 'nextjs-app' });
    expect(pickPackagesForTarget('inbox', target)).toEqual(['@notify/nextjs']);
  });
  ...
  it('skips already-installed packages', () => {
    const target = buildTarget({
      role: 'fullstack',
      framework: 'nextjs-app',
      installed: ['@notify/nextjs'],
    });
    expect(pickPackagesForTarget('full', target)).toEqual(['@notify/api']);
  });

  it('adds @react-email/components when @notify/framework is present and the goal includes workflows', () => {
    const target = buildTarget({
      role: 'api',
      framework: 'react',
      installed: ['@notify/framework'],
    });
    expect(pickPackagesForTarget('workflows', target)).toEqual(['@notify/api', '@react-email/components']);
  });

  it('does NOT pull @react-email/components when @notify/framework is absent (no-code path)', () => {
    const target = buildTarget({ role: 'api', framework: 'react' });
    expect(pickPackagesForTarget('workflows', target)).toEqual(['@notify/api']);
  });

  it('uses @notify/react-native in a React Native fullstack-equivalent workspace', () => {
    const target = buildTarget({
      role: 'web',
      framework: 'react',
      isReactNative: true,
    });
    expect(pickPackagesForTarget('inbox', target)).toEqual(['@notify/react-native']);
  });

  it('uses @notify/react + @notify/api in a non-Next.js fullstack workspace (Remix)', () => {
    const target = buildTarget({ role: 'fullstack', framework: 'remix', isReactBased: true });
    expect(pickPackagesForTarget('full', target)).toEqual(['@notify/react', '@notify/api']);
  });

  it('uses @notify/js + @notify/api in a fullstack workspace without React (SvelteKit / Nuxt)', () => {
    const target = buildTarget({ role: 'fullstack', framework: 'unknown', isReactBased: false });
    expect(pickPackagesForTarget('full', target)).toEqual(['@notify/js', '@notify/api']);
  });

  it('uses @notify/js in a non-React web workspace (Vue / Svelte / Solid)', () => {
    const target = buildTarget({ role: 'web', framework: 'unknown', isReactBased: false });
    expect(pickPackagesForTarget('inbox', target)).toEqual(['@notify/js']);
  });
...
describe('renderInstallCommand (smoke — covers the per-workspace command shape used by the install step)', () => {
  ...
  it('renders ONE pnpm command containing every package for a workspace', () => {
    ...
    expect(cmd).toBe('pnpm add @notify/nextjs @notify/api --ignore-workspace-root-check --filter @app/web');
  });
  ...
  it('renders ONE npm command with --workspace and applies --legacy-peer-deps for React 19', () => {
    ...
    expect(cmd).toBe('npm install @notify/react --workspace @app/web --legacy-peer-deps');
  });
  ...
  it('uses `yarn workspace <name> add` shape for yarn v1', () => {
    ...
    expect(cmd).toBe('yarn workspace @app/api add @notify/api --ignore-workspace-root-check');
  });
  ...
  it('drops the workspace filter when no monorepo name is provided', () => {
    ...
    expect(cmd).toBe('pnpm add @notify/react --ignore-workspace-root-check');
  });
});
