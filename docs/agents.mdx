---
title: "Notify Connect - Agent Communication Infrastructure (ACI)"
description: "Connect AI agents to Slack, Teams, WhatsApp, Telegram, and email. Route inbound messages, preserve conversation context, and deliver replies through Notify ACI."
sidebarTitle: Overview
---

Notify Connect is based on the [Agent Communication Infrastructure (ACI)](/agents/get-started/what-is-aci). It ingests channel webhooks, normalizes events, maintains conversation state, and delivers replies so you can run one agent across Slack, Microsoft Teams, WhatsApp, Telegram, and email without rebuilding each integration.
...
Users message the agent on a connected channel (DM, @mention, or email). Notify loads the conversation history and forwards it to your agent brain with the current turn. Replies go back to the same thread.
...
2. Notify ingests the webhook, normalizes the event, and resolves identity.
3. Notify forwards the event and conversation history to your agent brain.
4. Your agent returns a reply (and may wait on tool approval).
5. Notify delivers the reply to the correct thread.
6. The conversation is stored and visible in the [Notify dashboard](https://dashboard.novu.co/agents).
