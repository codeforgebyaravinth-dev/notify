...
description: "Enable external tools like Linear, GitHub, and Notion on your Notify managed agent by configuring MCP servers in the dashboard for tool-augmented replies."
---
...
<Steps>
  <Step title="Open the agent detail page">
    Open the [Notify dashboard](https://dashboard.novu.co) and go to your managed agent's detail page.
  </Step>
  ...
  <Step title="Wait for sync">
    Notify syncs the configuration to the AI platform.
  </Step>
</Steps>
...
| **Notify-managed OAuth** | Notify handles the OAuth flow. End users authorize on first use and Notify stores the tokens. |
...
<Steps>
  <Step title="Agent encounters a tool call">
    Notify prompts the user to authorize the tool before continuing.
  </Step>
  ...
  <Step title="Notify stores tokens">
    Notify stores the tokens securely and uses them on every subsequent turn.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Wait for removal">
    Notify removes the server from the AI platform and deletes any associated connections.
  </Step>
</Steps>
...
The exact list depends on your connector and Notify account tier. Common servers include:
