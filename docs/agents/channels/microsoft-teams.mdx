...
Use Microsoft Teams as a messaging channel for your agent. Notify keeps conversation context, shows typing during the turn, can update a plan card in place, can attach files on replies, and posts Adaptive Card approvals for gated tools.
...
A user messages the agent in Teams. Notify adds an eyes reaction, shows typing, updates a plan card while tools run, then replies with action buttons.
...
Teams provider credentials and app packaging are configured in the Notify dashboard when you add the Microsoft Teams provider.

## Connect component

Use `MsTeamsConnectButton` from `@notify/react` so end users can admin-consent and link Teams to your agent from your product UI. In subscriber mode, `autoLinkUser` defaults to `true` so the per-user Teams identity is linked after consent.
