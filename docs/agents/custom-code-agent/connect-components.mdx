...
Notify provides pre-built UI components for chat platforms like Slack, Microsoft Teams, Telegram and other communication platforms. Add these components so users can install the communication platform app in their workspace and connect it to your agent.

## Slack connect button

`SlackConnectButton` is a pre-built UI component in the `@notify/react` SDK that connects an agent to a Slack workspace. Check out the example below.
...
Add the Notify SlackConnectButton from @notify/react to my app so each of my end users can connect "YOUR_AGENT_NAME" to their own Slack workspace.

Context: The Slack integration already exists in Notify. This is purely a frontend code integration - do NOT run the Notify CLI or the agent-onboarding flow.

Requirements:
- Install @notify/react with my project's package manager.
- Render `<SlackConnectButton />` inside a `<NotifyProvider>` configured for the currently signed-in end user.
...
`MsTeamsConnectButton` is a pre-built UI component in the `@notify/react` SDK that connects an agent to a user's Microsoft Teams. In subscriber mode, `autoLinkUser` defaults to `true` so the per-user Teams identity is linked after admin consent.

```tsx
import { NotifyProvider, MsTeamsConnectButton } from '@notify/react';
...
const MsTeamsConnectButtonComponent = () => {
  ...
  return (
    <NotifyProvider subscriberId={subscriberId} applicationIdentifier="application-identifier">
...
Add the Notify MsTeamsConnectButton from @notify/react to my app so each of my end users can connect "YOUR_AGENT_NAME" to their own Microsoft Teams.

Context: The Microsoft Teams integration already exists in Notify. This is purely a frontend code integration - do NOT run the Notify CLI or the agent-onboarding flow.

Requirements:
- Install @notify/react with my project's package manager.
- Render `<MsTeamsConnectButton />` inside a `<NotifyProvider>` configured for the currently signed-in end user.
- Use applicationIdentifier="YOUR_APPLICATION_IDENTIFIER" and integrationIdentifier="YOUR_INTEGRATION_IDENTIFIER" with connectionMode="subscriber". Store applicationIdentifier in an environment variable rather than hardcoding it.
...
| `subscriberId` | `string` | Optional. Defaults to the `NotifyProvider` subscriber. |
...
`TelegramConnectButton` is a pre-built UI component in the `@notify/react` SDK that links a subscriber's Telegram chat to your agent. It authenticates with the subscriber JWT from `NotifyProvider`, so no secret key is exposed in the browser.
...
```tsx
import { NotifyProvider, TelegramConnectButton } from '@notify/react';
...
Add the Notify TelegramConnectButton from @notify/react to my app so each of my end users can connect "YOUR_AGENT_NAME" to their own Telegram chat.

Context: The Telegram integration already exists in Notify. This is purely a frontend code integration - do NOT run the Notify CLI or the agent-onboarding flow.

Requirements:
- Install @notify/react with my project's package manager.
- Render `<TelegramConnectButton />` inside a `<NotifyProvider>` configured for the currently signed-in end user.
...
| `subscriberId` | `string` | Optional. Subscriber to link. Defaults to the `NotifyProvider` subscriber. |
