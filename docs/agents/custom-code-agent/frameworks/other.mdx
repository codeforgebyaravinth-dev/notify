...
description: "API patterns for @notify/framework with any LLM stack: OpenAI SDK, Anthropic SDK, Mastra, or custom code."
sidebarTitle: Other
---


Already connecting an agent? For Mastra, start with [Connect Mastra to Slack](/agents/get-started/mastra). This page is the generic Framework API for stacks without a dedicated Notify adapter.

Import `agent` from `@notify/framework` when you are not using `@notify/framework/ai-sdk` or `@notify/framework/langchain`. Call your LLM, orchestration library, or business logic yourself, then reply through Notify's handler API. Use this path for Mastra, the OpenAI SDK, Anthropic SDK, or any stack Notify does not ship a dedicated adapter for.
...
Bring your own LLM or agent library - Notify does not require a specific one.

## Minimal agent

Import `agent` from `@notify/framework`, call your logic in `onMessage`, and reply by returning a string or calling `ctx.reply()`:
...
<Columns cols={2}>
  <Card icon="boxes" href="/agents/get-started/mastra" title="Connect Mastra">
    Call Mastra generate() from a Notify handler.
  </Card>
