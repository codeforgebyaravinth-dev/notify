...
description: "API reference for @notify/framework/ai-sdk: return types, tools, MCP, and approval gating with the Vercel AI SDK."
sidebarTitle: AI SDK
---
...
`@notify/framework/ai-sdk` lets you write agent handlers with the [Vercel AI SDK](https://ai-sdk.dev/). Return an AI SDK result straight from `onMessage` and Notify delivers the text - including tool loops and approval gating - to the conversation.
...
<CodeGroup>
```bash title="OpenAI"
npm install @notify/framework ai @ai-sdk/openai
```
```bash title="Anthropic"
npm install @notify/framework ai @ai-sdk/anthropic
```
```bash title="Google"
npm install @notify/framework ai @ai-sdk/google
```
```bash title="Groq"
npm install @notify/framework ai @ai-sdk/groq
```
</CodeGroup>

`@notify/framework` and `ai` are required; pick one provider package. Examples on this page use OpenAI - see the [AI SDK providers list](https://ai-sdk.dev/providers/ai-sdk-providers) for others.

## Minimal agent

Import `agent` and `toModelMessages` from `@notify/framework/ai-sdk`, then return `generateText()` from `onMessage`:
...
| Return | Behavior |
| --- | --- |
| `generateText(...)` | Notify delivers the generated text |
| `streamText(...)` | Notify waits for the full response, then delivers the final text (same outcome as `generateText`) |
...
<Note>
  Returning `streamText(...)` behaves the same as returning `generateText(...)` - Notify waits for the full response, then posts the completed text.
</Note>
...
Set `needsApproval: true` on an AI SDK `tool()`. When the model calls it, Notify posts the Approve / Deny card and pauses the turn. After the user decides, Notify re-runs `onMessage` - `toModelMessages(ctx.history)` now includes the decision, so the tool loop continues with no extra code from you.
...
When a returned `generateText()` / `streamText()` result rejects - for example the provider errors or stream consumption fails - the failure propagates to the same dispatch boundary as a thrown handler error. Register `onError` on the agent to log, suppress user notification, send custom copy, or fall back to Notify's generic message.
...
Notify does not inject MCP credentials into `ctx`. Resolve tokens in your handler - for example, look up a per-subscriber token you stored after your own OAuth flow using `ctx.subscriber?.subscriberId`.
