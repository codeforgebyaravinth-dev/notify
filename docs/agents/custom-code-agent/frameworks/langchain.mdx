...
description: "API reference for @notify/framework/langchain: agent configs, tools, approval gating, and custom invoke."
...
`@notify/framework/langchain` lets you write agent handlers with [LangChain](https://docs.langchain.com/). Return a `LangChainAgentConfig` from `onMessage` and Notify runs `createAgent().invoke()` for you - including tool approval gating and resume from `ctx.history`.
...
<CodeGroup>
```bash title="OpenAI"
npm install @notify/framework langchain @langchain/core @langchain/openai
```
```bash title="Anthropic"
npm install @notify/framework langchain @langchain/core @langchain/anthropic
```
```bash title="Google"
npm install @notify/framework langchain @langchain/core @langchain/google-genai
```
</CodeGroup>

`@notify/framework`, `langchain`, and `@langchain/core` are required; pick one provider package. Examples on this page use OpenAI model strings - see the [LangChain integrations catalog](https://docs.langchain.com/oss/javascript/integrations/providers) for others.

## Minimal agent

Import `agent` from `@notify/framework/langchain`, then return a `LangChainAgentConfig` from `onMessage`:
...
| Return | Behavior |
| --- | --- |
| `LangChainAgentConfig` | Notify runs `createAgent()` and delivers the final assistant text |
| `{ messages: BaseMessage[] }` | Notify delivers the final assistant text from your pre-invoked agent or graph |
...
`npx notify connect --runtime langchain` scaffolds this for you (and adds the provider package you selected).
...
```typescript
import { toLangChainMessages } from '@notify/framework/langchain';
...
- Pass `system` only when you are **not** setting `system` on `LangChainAgentConfig` - Notify forwards that field to `createAgent({ prompt })` for you.
- Tool approval cycles in history are mapped automatically, so auto-resume (below) works.

## Invoke your own agent

If you already run a LangChain agent or graph in your handler, return its messages and Notify delivers the final text - tool approval is **not** managed on this path:

```typescript
import { createAgent } from 'langchain';
import { agent, toLangChainMessages } from '@notify/framework/langchain';
...
Set `needsApproval` on `LangChainAgentConfig`. When the model calls a gated tool, Notify posts the Approve / Deny card and pauses the turn. After the user decides, Notify re-runs `onMessage` - `toLangChainMessages(ctx.history)` replays the approval cycle so the agent continues with no extra code from you.
...
When Notify runs `createAgent().invoke()` for a returned `LangChainAgentConfig` and the invoke rejects - for example the provider errors or the model call fails - the failure propagates to the same dispatch boundary as a thrown handler error. The same applies when you `await` your own agent or graph in `onMessage` and the invoke throws before you return. Register `onError` on the agent to log, suppress user notification, send custom copy, or fall back to Notify's generic message.
...
export const supportAgent = agent('support-bot', {
  ...
  onError: async (error, ctx) => {
    ...
    // return nothing → generic user copy from Notify
...
<Columns cols={2}>
  ...
  <Card icon="code" href="/agents/custom-code-agent/frameworks/other" title="Other frameworks">
    Wire handlers without a Notify runtime adapter.
  </Card>
</Columns>
