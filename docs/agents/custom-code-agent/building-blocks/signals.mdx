---
title: "Signals"
description: "Use signals in custom code agent handlers to update metadata, trigger Notify workflows, and mark conversations resolved from your agent runtime."
---
...
Use `ctx.reply()` when the user should see something. Use signals when you only need to persist data, trigger a Notify workflow, or close the conversation.
...
Resolve and other signals are channel-agnostic. Notify applies them to the conversation regardless of provider. See [Channels overview](/agents/channels/overview).
...
| Trigger | Fire a Notify workflow (email, push, SMS, multi-step) | `ctx.trigger(workflowId, options)` |
...
Start any Notify workflow from a handler, the same workflows you use for email, push, or SMS elsewhere in your account.
