...
Use `url` for larger files. Notify fetches public HTTP(S) URLs server-side. Use `data` for small generated files in memory.
...
Add `"jsxImportSource": "@notify/framework"` to `tsconfig.json`, then return JSX from a handler or pass it to `ctx.reply()`:
