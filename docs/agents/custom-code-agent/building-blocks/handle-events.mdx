...
description: "Write event handlers for your custom code agent and understand the context object that Notify passes on every turn, including messages, users, and providers."
---


Notify gives your handler the same building blocks whether the message came from Slack, email, or another provider, and whether you call an LLM or plain TypeScript.
...
| `onError` | A turn fails (handler throw, promise rejection, delivery error) | Log, suppress, send custom copy, or rely on Notify's generic reply |

Handlers are where the communication layer connects to your application logic. For example, an `onMessage` handler receives the user's message, passes conversation context to an LLM or custom function, and sends the response back through Notify.
...
The context object is how your code talks to Notify. You do not call Slack, Teams, or email APIs directly in the handler.
...
sequenceDiagram
    ...
    Provider->>Notify: Platform webhook
    Notify->>Notify: Map thread to conversation
    Notify->>Bridge: Call onMessage with context
    Bridge->>Agent: Pass message and history
    Agent->>Bridge: Return reply and signals
    Bridge->>Notify: ctx.reply and signals
    Notify->>Provider: Deliver reply to thread
    Notify->>Notify: Persist conversation state
...
<Steps>
  ...
  <Step title="Notify maps the thread">
    Notify maps the provider thread to a conversation and resolves the platform user to a subscriber, when possible.
  </Step>

  <Step title="Notify calls onMessage">
    Notify calls the agent's `onMessage` handler with the context object.
  </Step>
  ...
  <Step title="Notify delivers the reply">
    The agent response is posted back to the original provider thread.
  </Step>

  <Step title="Notify records conversation state">
    Notify records messages, participants, metadata, signals, and conversation status.
  </Step>
</Steps>

The same agent logic works across all connected providers because Notify handles the provider-specific communication layer. Connecting a new provider does not require changing your agent code.
...
```typescript
import { agent, toModelMessages } from '@notify/framework/ai-sdk';
...
Return `generateText()` or `streamText()` from the handler - Notify delivers the model output to the thread. See [AI SDK](/agents/custom-code-agent/frameworks/ai-sdk).
...
Return a `LangChainAgentConfig` from the handler - Notify runs the agent and delivers the final text. See [LangChain](/agents/custom-code-agent/frameworks/langchain).

  </Tab>
  <Tab title="Custom code">

```typescript
import { agent } from '@notify/framework';
...
Inbound messages can include file attachments when the platform supports them. Notify normalizes files into `message.attachments` with short-lived signed URLs. Keep in mind:
...
When a turn fails - your handler throws, a returned promise rejects, or message delivery fails inside the handler - Notify runs an error pipeline on the bridge before the turn ends:

1. Log the failure locally on your bridge
2. Call `onError` if you registered one
3. If `onError` does not handle the failure, auto-report `{ error: true }` to Notify
4. Notify delivers safe generic copy to the user in the thread
...
| --- | --- |
| nothing / `undefined` | Auto-report - Notify sends the generic message |
| `{ suppress: true }` | No user-visible reply (failure is logged only) |
| `string` / JSX `Card` | Custom reply delivered through `ctx.reply()` |

The first argument is an `AgentError` (use `toAgentError()` from `@notify/framework` if you normalize errors in your own code). Reply delivery failures surface as `AgentDeliveryError`, a subclass of `AgentError`, with `statusCode` and `responseBody` for debugging.
...
When `onError` returns message content, Notify posts it like a normal reply. When it returns nothing, the framework auto-reports the turn failure and Notify sends the generic message.
...
<Note>
  Reporting turn failures is best-effort. If the bridge cannot reach Notify, the failure is logged locally and is not re-thrown. Failures inside `onError` itself are logged and do not recurse.
</Note>
