...
description: "Create a managed agent on Claude Platform for AWS with the Notify CLI. Provide region, workspace ID, and API key, then connect Slack."
sidebarTitle: AWS Claude Managed
---

Create a managed agent on **Claude Platform for AWS** and connect it to Slack with the Notify CLI. No bridge server is required.
...
Notify verifies the credentials and creates an AWS Claude runtime integration in your environment.
</Step>
...
Use the Notify CLI (`npx notify connect`) with AWS Claude Managed Agent. Provide AWS region, workspace ID (wrkspc_…), and API key. Connect Slack as the first channel.
