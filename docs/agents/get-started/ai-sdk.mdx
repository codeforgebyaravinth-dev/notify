...
description: "Add @notify/framework/ai-sdk to an existing app, connect Slack, and return generateText from onMessage. Same handler works on Teams, WhatsApp, Telegram, and email."
sidebarTitle: Connect AI SDK
boost: 3
---

Connect a [Vercel AI SDK](https://ai-sdk.dev/) agent to Notify Connect. Return `generateText()` from `onMessage`; Notify delivers the reply on Slack. After you add other providers, the same handler serves Teams, WhatsApp, Telegram, and email.
...
- A [Notify account](https://dashboard.novu.co)
- Node.js 22+
...
In the [Notify dashboard](https://dashboard.novu.co):
...
The token is used once to create the Slack app. Notify does not store it. Screenshots: [Create a Slack app](/agents/channels/slack#create-a-slack-app).

Alternatively run `npx notify connect`, choose **AI SDK** and **Slack**, and skip creating a starter project when prompted.
</Step>
...
```bash
NOVU_SECRET_KEY=your-notify-secret-key
...
Create `app/notify/agents/<your-identifier>.ts` (or the equivalent path). Match `agent('...')` to the dashboard **Identifier**.
...
```bash
npx notify dev --port 4000
```

Or `npm run dev:notify` if that script exists. Keep the process running.
...
<AccordionGroup>
  ...
  <Accordion title="Does Notify run my AI SDK code?">
    No. Your app serves the bridge endpoint. Notify forwards the inbound event and conversation context, then delivers whatever the handler returns.
  </Accordion>
...
<Prompt description="Connect my Vercel AI SDK agent to Slack with Notify" icon="plug" actions={["copy", "cursor"]}>
# Connect a Vercel AI SDK agent to Slack with Notify Connect

Follow https://docs.novu.co/agents/get-started/ai-sdk

## Goal

Add Notify bridge + AI SDK handler to this repo and connect Slack. Prefer the existing project over scaffolding a new one.

## Rules

ALWAYS:

- Match `agent('id')` to the Notify dashboard agent Identifier
...
- Hardcode API keys
- Call `ctx.reply()` when returning `generateText()` (Notify delivers the result)
...
<Columns cols={2}>
  ...
  <Card icon="link" href="/agents/get-started/langchain" title="Connect LangChain">
    Same flow with @notify/framework/langchain.
  </Card>
</Columns>
