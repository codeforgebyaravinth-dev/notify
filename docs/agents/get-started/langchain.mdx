...
description: "Add @notify/framework/langchain to an existing app, connect Slack, and return a LangChainAgentConfig from onMessage."
sidebarTitle: Connect LangChain
boost: 3
---

Connect a [LangChain](https://docs.langchain.com/) agent to Notify Connect. Return a `LangChainAgentConfig` from `onMessage`; Notify invokes the agent and delivers the reply on Slack. The same handler works on other providers after you link them.
...
Alternatively run `npx notify connect`, choose **LangChain** and **Slack**, and skip creating a starter project when prompted. Screenshots: [Create a Slack app](/agents/channels/slack#create-a-slack-app).
</Step>
...
Create `app/notify/agents/<your-identifier>.ts`. Match `agent('...')` to the dashboard **Identifier**.
...
Export from `app/notify/agents/index.ts`:

```typescript
export { supportBot } from './support-bot';
```

Returning `{ model, system }` is a `LangChainAgentConfig`. Notify calls `createAgent().invoke()` and posts the final text. Notify maps `ctx.history` on this path.
...
<AccordionGroup>
  ...
  <Accordion title="Does Notify run LangChain for me?">
    When you return a `LangChainAgentConfig`, Notify invokes `createAgent()` with your model, system prompt, and tools, then posts the final text.
  </Accordion>
...
<Prompt description="Connect my LangChain agent to Slack with Notify" icon="plug" actions={["copy", "cursor"]}>
# Connect a LangChain agent to Slack with Notify Connect

Follow https://docs.novu.co/agents/get-started/langchain

## Goal

Add Notify bridge + LangChain config handler to this repo and connect Slack. Prefer the existing project over scaffolding a new one.
...
- Return a LangChainAgentConfig so Notify can invoke the agent
...
<Columns cols={2}>
  ...
  <Card icon="sparkles" href="/agents/get-started/ai-sdk" title="Connect AI SDK">
    Same flow with @notify/framework/ai-sdk.
  </Card>
</Columns>
