...
description: "Add @notify/framework to an existing Mastra app, connect Slack, and return generate() output from onMessage."
sidebarTitle: Connect Mastra
boost: 3
---

Connect a [Mastra](https://mastra.ai/) agent to Notify Connect via `@notify/framework`. Call `generate()` in `onMessage` and return the text. Notify delivers the reply on Slack. The same handler works on other providers after you link them.

There is no `@notify/framework/mastra` adapter yet. Use the base Framework package and return text from `onMessage` (or call `ctx.reply()`).
...
Alternatively run `npx notify connect`, choose **Custom code** and **Slack**, and skip creating a starter project when prompted.
</Step>
...
```bash
npm install @notify/framework @mastra/core
```
...
<Step>
## Add the Mastra agent and Notify handler
...
Match `agent('...')` to the dashboard **Identifier**. Export the Notify agent from your agents index.
</Step>
...
<AccordionGroup>
  ...
  <Accordion title="Does Notify have a Mastra adapter?">
    Not yet. Use `@notify/framework` and return text from `onMessage`. AI SDK and LangChain have dedicated adapters if you prefer those stacks.
  </Accordion>
...
<Prompt description="Connect my Mastra agent to Slack with Notify" icon="plug" actions={["copy", "cursor"]}>
# Connect a Mastra agent to Slack with Notify Connect

Follow https://docs.novu.co/agents/get-started/mastra

## Goal

Add Notify bridge + Mastra handler to this repo and connect Slack. Prefer the existing project over scaffolding a new one.

## Rules

ALWAYS:

- Match Notify `agent('id')` to the dashboard agent Identifier
...
- Hardcode API keys
- Import `@notify/framework/ai-sdk` or `@notify/framework/langchain` for this path
