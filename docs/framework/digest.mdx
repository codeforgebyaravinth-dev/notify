---
title: 'Digest Action'
description: "Collect multiple workflow events into a single notification with the Notify Framework digest step, using time windows and grouping keys."
---
...
Currently, Notify does not support two digest steps in a workflow. However, two digest behaviours can be achieved by creating second workflow with digest step, add all other steps after the digest step and trigger the second workflow from the first workflow in [custom step](/framework/custom).
