---
title: 'NestJS'
description: "Get started with Notify Framework in a NestJS application by scaffolding a bridge module, defining a workflow, and triggering your first notification."
---


In this guide, we will add a Notify [Bridge Endpoint](/framework/endpoint) to a NestJS application and send our first test workflow.
...
This package provides all the necessary tools to build and manage your notification workflows.
    ...
    <Step title="Add the NovuModule to your application">
        The `NovuModule` is a NestJS module that registers the Notify Endpoint in your application.
...
```
    ...
    <Step>
        ...
        Add a `notify` folder in your `src` folder as such ```src/notify/workflows.ts``` that will contain your workflow definitions.
...
```
    ...
    <Step>
        ...
        Start your NestJS application with the Notify Endpoint configured.

        If your NestJS application is running on other than `4000` port, restart the `npx notify dev` command with the port:

        ```tsx
        npx notify@latest dev --port <YOUR_NESTJS_APPLICATION_PORT>
        ```
    </Step>
    <Step>
        ### Test your endpoint
