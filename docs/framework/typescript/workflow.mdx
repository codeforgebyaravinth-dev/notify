---
title: 'Workflow'
description: "Reference the Notify Framework workflow interface: workflow IDs, payload schemas, tags, preferences, control values, and step composition."
---
...
With Workflow channel preferences, you can control the default delivery preference for a channel and whether a subscriber can change it. Notify will show the subscriber preferences in `<Inbox/>` component. Subscribers can enable and disable any active channel in the workflow.
...
<Note>
  These preferences can also be controlled from the Notify Dashboard per workflow. To do so, click on
