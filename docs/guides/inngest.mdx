---
title: 'Inngest'
description: "Integrate Inngest with Notify to trigger notification workflows from background jobs and durable functions. Covers setup, event sending, and best practices."
---


## Why Use Notify with Inngest?
...
- Configure Notify and Inngest in your project
- Manage notification subscribers via Notify's API
...
<Steps>
  <Step>
    Install Notify's SDK in our project:
    ```bash
    npm i @notify/api
    ```
  </Step>
  ...
  <Step>
    ...
    Import Notify's SDK and provide the credentials to initialize the Notify instance.
    ...
    notify = Notify(secret_key=os.getenv("NOVU_SECRET_KEY", ""))
    ```
      </Tab>
      <Tab title="Go">
    ```go
    ...
    notify := novugo.New(novugo.WithSecurity(os.Getenv("NOVU_SECRET_KEY")))
    ```
      </Tab>
      <Tab title="PHP">
    ```php
    ...
    var notify = new NovuSDK(secretKey: Environment.GetEnvironmentVariable("NOVU_SECRET_KEY")!);
    ```
      </Tab>
      <Tab title="Java">
    ```java
    ...
    For the sake of this guide, we will create a new dedicated functions for 2 common Notify actions:
...
You can trigger Notify notification workflows directly from your Ingest function. This allows you to send notifications based on events or completions within your background jobs.

To trigger a Notify workflow, you'll use the Notify Node.js SDK within your Inngest's `createFunction` method.

### Core Requirements

When calling Notify's `trigger` method, you must provide the following information:

1. `workflowId` **(string):** This identifies the specific notification workflow you want to execute.
    - **Note:** Ensure this workflow ID corresponds to an existing, active workflow in your Notify dashboard.
2. `to` **(object):** This object specifies the recipient of the notification. At a minimum, it requires:
    ...
    **Note:** If a subscriber with this `subscriberId` doesn't already exist in Notify, Notify will automatically create one when the workflow is triggered. You can also pass other identifiers like `email`, `phone`, etc., within the `to` object, depending on your workflow steps.
    

### Basic Trigger Example

Here's a simple Inngest function that triggers a Notify workflow when the function is being invoked. It assumes the function invoke receives a `userId` and `email` in its payload.
...
The `payload` is an object containing key-value pairs that you can reference within your Notify notification templates using Handlebars syntax (e.g., `{{ name }}`, `{{ order.id }}`).
...
Let's modify the previous function to accept more data (like a user's `name`) in its own payload and pass relevant parts to the Notify `trigger` payload.
...
## Add a New Subscriber to Notify

Before sending notifications, Notify needs to know *who* to notify. That's where subscribers come in. A **subscriber** in Notify represents a user (or entity) that can receive notifications through one or more channels (in-app, email, SMS, etc.).

You can create a new subscriber programmatically from a Inngest's task using Notify's SDK.

---

### When Should You Create Subscribers?

Create or update a subscriber in Notify when:
...
<Note>
  If you trigger a workflow with a `subscriberId` that doesn't exist, Notify will auto-create the subscriber. However, doing it explicitly ensures full control over subscriber data.
</Note>

### Create a Subscriber in a Task

Below is a task that creates a Notify subscriber using a Inngest task. It also supports optional fields like `phone` and `avatar`.
...
4. You have a Notify account, API Key (`NOVU_SECRET_KEY` environment variable), and a Notify workflow created with the ID `video-processed-notification`. 
...
export const processVideoAndNotify = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // --- Step 4: Send Completion Notification via Notify ---
    ...
    await step.run("send-completion-notification", async () => {
      ...
      console.log(`Attempting to send Notify notification for videoId: ${videoId} to user: ${userId}`);
      ...
      try {
        ...
        console.log(`Notify workflow 'video-processed-notification' triggered successfully for videoId: ${videoId}`, triggerResult);
        return { success: true, novuPayload: novuPayload, triggerResult };

      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error triggering Notify';
        console.error(`Failed to trigger Notify workflow for videoId: ${videoId}: ${errorMessage}`, error);
        ...
        // throw new Error(`Notify trigger failed: ${errorMessage}`); // Uncomment to make it retry
...
The Notify trigger call inside the implementation example maps to these SDK equivalents:
...
```
  ...
  <Tab title="PHP">
```php
$notify->trigger(triggerEventRequestDto: new Components\TriggerEventRequestDto(
    workflowId: 'video-processed-notification',
    to: new Components\SubscriberPayloadDto(subscriberId: $userId, email: $email),
    payload: [
        'userName' => $userName,
...
Once you've added custom fields (like `firstName`, `userName`, or `phone`) to a subscriber, you can use them in Notify templates using Handlebars:
...
- Store your Notify `subscriberId` as part of your user model - so you always have a reference.
