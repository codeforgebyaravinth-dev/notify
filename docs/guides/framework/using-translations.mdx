...
description: "Translate Notify Framework workflow step content into multiple languages with translation keys, locales, and per-subscriber locale resolution."
---


Notify Framework workflows are code-first: workflow definitions and content are expressed in code rather than in the dashboard. Internationalization (i18n) is implemented in the application layer - translations are defined there and resolved at runtime - so that workflow step content (e.g. subject and body fields) can be locale-aware.
...
- Create the workflow definition by importing the `workflow` function from the `@notify/framework` library.
    ...
    await step.email(
      ...
      async (controls) => {
        ...
        const subject = translate("welcomeEmailSubject", {
          username: subscriber?.firstName || "Notify",
        });
...
- [Sync the workflow](/framework/deployment/syncing) to the Notify cloud.
