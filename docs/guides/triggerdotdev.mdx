---
title: 'Trigger.dev'
description: "Integrate Notify with Trigger.dev to send notifications from background jobs and scheduled tasks. Covers setup, subscriber sync, and practical workflow examples."
---


This guide walks you through integrating [Notify](https://novu.co/), the open-source notification infrastructure, with [Trigger.dev](https://trigger.dev/), a powerful open-source background jobs framework for TypeScript.

By combining Notify's robust notification workflows with Trigger.Dev's event-driven background job system allows you to send notifications across multiple channels (in-app, email, SMS, push, etc.) in response to events or background tasks - all within a seamless developer experience.

Whether you're processing payments, handling user onboarding, or running scheduled tasks, Trigger.dev lets you define workflows with fine-grained control and real-time observability. Notify plugs into those workflows to manage notification content and delivery, ensuring your service / product-to-user communication has a standard. 

## What You'll Learn

In this guide, you'll learn how to:

- Set up both Notify and Trigger.dev in your project
- Create and update subscribers via Notify's API
- Trigger Notify notification workflows from a Trigger.dev job
...
<Steps>
  <Step>
    Install Notify's SDK in your project:
    ```bash
    npm i @notify/api
    ```
  </Step>
  ...
  <Step>
    ...
    ```
      ...
      <Tab title="PHP">
    ```php
    $notify = notify\Notify::builder()->setSecurity(getenv('NOVU_SECRET_KEY'))->build();
    ```
      </Tab>
      <Tab title=".NET">
    ```csharp
    ...
    ```
      ...
      <Tab title="Java">
    ```java
    Notify novu = Notify.builder().secretKey(System.getenv("NOVU_SECRET_KEY")).build();
    ```
      </Tab>
    </Tabs>
  </Step>
...
For the sake of this guide, we will create a new dedicated task for 2 common Notify actions:
...
Here's a simple Trigger.dev task that triggers a Notify workflow when the task runs.
...
<Columns cols={2}>
  ...
  <Card title="Notify Trigger Payload">
    This is the data you *specifically send to Notify* via the `payload` property in the `novu.trigger()` call. It's used for populating variables in your notification templates.
  </Card>
</Columns>
...
This example demonstrates how to build a reliable AI content generation system that keeps users informed throughout the process using Notify notifications.
...
**Notification Workflows You Would Need in Notify**
...
Notify trigger calls in the AI tasks example:
...
1. **Create two notification workflows in Notify:**
...
Notify trigger calls in the video processing example:
...
Before triggering notifications, Notify needs to know *who* to notify. That's where subscribers come in. A **subscriber** in Notify represents a user (or entity) that can receive notifications through one or more channels (in-app, email, SMS, etc.).
...
Once you've added custom fields to a subscriber, you can use them in Notify templates using Handlebars:
...
<AccordionGroup>
  <Accordion title="Data Management Best Practices">
    ...
    - Use your internal `userId` as the `subscriberId` in Notify
  ...
  <Accordion title="Proactive Data Synchronization">
    ...
    - Updating Notify whenever user contact info changes
