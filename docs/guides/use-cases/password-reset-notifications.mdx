...
description: "Build password reset, email verification, and authentication alert workflows with Notify using email and in-app channels, subscribers, and secure token payloads."
---


Send password reset and authentication alert notifications through Notify by triggering a workflow when your auth provider emits a security event. Notify delivers the reset link or alert via email, in-app, or SMS depending on your workflow configuration.
...
Use webhook guides to connect your auth provider to Notify:

- [Auth0 webhook integration](/guides/webhooks/auth0)
- [Clerk webhook integration](/guides/webhooks/clerk)

Both patterns verify the webhook signature, map the auth event to a Notify workflow, and pass user details as the subscriber payload.
...
<AccordionGroup>
  <Accordion title="Should I store reset tokens in Notify?">
    No. Generate and validate reset tokens in your auth system. Pass only the reset URL and metadata to Notify as workflow payload variables.
  </Accordion>
