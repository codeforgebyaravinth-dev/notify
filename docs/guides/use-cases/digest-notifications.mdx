...
sidebarTitle: 'Digest'
description: 'Batch and summarize notifications with Notify digest steps. Send digests like "10 new comments" instead of individual alerts, with configurable windows and keys.'
---


Batch related notifications into a single digest message using Notify's digest step. Instead of sending one notification per event, Notify collects events over a time window and delivers a summarized message such as "10 people liked your post."

## How does digest work in Notify?

Add a [digest step](/platform/workflow/add-and-configure-steps/configure-action-steps/digest) to your workflow before the channel step. Notify collects events that match the digest key within the configured window, then renders a single notification with aggregated content.
...
Trigger the same workflow for each individual event. Notify handles batching automatically based on the digest step configuration and digest key.
