---
title: 'Stripe'
description: "Integrate Stripe webhooks with Notify notifications in a Next.js application. Trigger workflows on payments, subscriptions, invoices, and refund events."
---
...
<Note>
  You can also clone this repository: https://github.com/novuhq/stripe-to-notify-webhooks
</Note>
...
```
npm install @notify/api @clerk/nextjs @stripe
```
...
To prevent users from receiving duplicate emails, we need to disable email delivery by Stripe for the notifications handled by Notify.

1. In your Stripe Dashboard, navigate to the **Settings**.

2. Under the **Product Settings** section, navigate to the **Billing** tab.

3. Toggle **off** delivery of the events you want to handle with Notify.

This ensures that Stripe does not send duplicate emails, allowing Notify to manage the notifications seamlessly.
