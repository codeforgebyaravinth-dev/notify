---
title: 'Clerk'
description: "Integrate Clerk webhooks with Notify notifications in a Next.js app. Trigger workflows on signups, session events, and organization membership changes."
---
...
4. Trigger the corresponding **Notify notification workflow**.

<Note>
  You can also clone this repository: https://github.com/novuhq/clerk-to-notify-webhooks
</Note>
...
- A **Notify account** ([Sign up here](https://novu.com/signup)).
...
```
npm install svix @notify/api @clerk/nextjs
```
...
import { triggerWorkflow } from '@/app/utils/notify'

// Single source of truth for all supported Clerk events and their corresponding Notify workflows
...
const EVENT_TO_WORKFLOW_MAPPINGS = {
    ...
    // Clerk webhook event type -> Notify workflowId
...
This mapping defines how Clerk webhook events are associated with Notify workflows.
...
This function determines the workflow ID by mapping the Clerk webhook event type to the Notify workflow ID.
...
## Add Notify Workflow Notification Trigger Function
...
var notify = new NovuSDK(secretKey: "<YOUR_SECRET_KEY_HERE>");
...
## Add or create Notify workflows in your Notify dashboard

In Notify, a webhook event - such as a user being created or updated - can trigger one or more workflows, depending on how you want to handle these events in your application. 
...
The Notify dashboard allows you to either create a custom workflow from scratch or choose from pre-built templates to streamline the process.

**Steps to Create a Workflow**

Follow these steps to set up your workflow(s) in the Notify dashboard:
...
<Tab title="Use a Workflow Template">
   ...
   Browse the workflow template store in the Notify dashboard. If a template matches your use case (e.g., "User Onboarding"), select it and proceed to customize it.
...
<Tab title="Code-First Workflow (Notify Framework)">

   If you prefer a more code-based approach, you can create a workflow using the Notify Framework.
   
   <Card title="Notify Framework" icon="square-code" href="/framework">
   <p>
      The Notify framework allows you to build and manage advanced notification workflows with code, and expose no-code controls for non-technical users to modify.
   </p>
   </Card>
...
### Configure the Workflow  
   ...
   - For a code-first workflow, you can use the Notify Framework to build your workflow right within your code base.
...
To prevent users from receiving duplicate emails, we need to disable email delivery by Clerk for the notifications handled by Notify.

1. Navigate to the **Emails** section in the Clerk Dashboard.

   ![Clerk's Dashboard 1](/images/guides/clerk/clerk-email-dashboard.png)

2. Select any **email.created** event that you want Notify to handle.
3. Toggle **off** email delivery for the selected event.

   ![Clerk's Dashboard 2](/images/guides/clerk/clerk-email-dashboard2.png)

This ensures that Clerk does not send duplicate emails, allowing Notify to manage the notifications seamlessly.
