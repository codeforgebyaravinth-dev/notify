---
title: 'Auth0 Webhook Integration with Notify'
sidebarTitle: 'Auth0'
description: "Trigger Notify notification workflows from Auth0 events such as password reset, signup, and login alerts by forwarding webhooks to your Notify workflows."
---


Trigger Notify notification workflows when Auth0 events occur by receiving Auth0 log stream or webhook events in your backend and calling the Notify Trigger API.

## How does the Auth0 and Notify integration work?
...
3. Your handler verifies the event and calls Notify's [Trigger event](/api-reference/events/trigger-event) API.
4. Notify delivers the notification through your configured workflow channels.

## Prerequisites

- An Auth0 tenant with log streams or Actions configured
- A Notify account with a workflow for each auth event
- A backend endpoint to receive Auth0 events

## How do I trigger a Notify workflow from Auth0?

<Steps>
  <Step title="Create auth workflows in Notify">
  ...
  <Step title="Handle the webhook and trigger Notify">
    Verify the Auth0 event, then trigger the matching Notify workflow:
...
<AccordionGroup>
  ...
  <Accordion title="Where should I verify Auth0 webhook signatures?">
    Always verify signatures in your backend before triggering Notify workflows. Never expose your Notify secret key in client-side Auth0 Actions.
  </Accordion>
</AccordionGroup>
