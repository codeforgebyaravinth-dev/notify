---
title: 'Run Notify in local machine'
description: 'Prerequisites and steps to run Notify in local machine. Learn how to set up Notify on your local environment for testing and development.'
---
...
<Note>
  We recommend having at least 8GB of RAM to run Notify on a local machine as Notify has multiple
...
<Note>
  The legacy Widget and `@notify/notification-center` apps have been removed. Use the [Inbox component](/platform/inbox) (`@notify/react`) instead.
</Note>

## Set up your environment variables

If you used `npm run setup:project` or Jarvis, default `.env` files are created automatically. To test certain parts of Notify or run it in production mode, you may need to change some values. These are the main environment variables:
...
pnpm --filter @notify/api-service test:e2e:notify-v2
```

The tests create a new instance of Notify and a test database, then remove the test database when finished.
...
Jarvis includes a **Development environment setup** option that runs [this script](https://github.com/novuhq/novu/blob/next/scripts/dev-environment-setup.sh). It tries to install OS dependencies needed to run Notify locally before `npm run setup:project`. You still need `git` and Node.js installed beforehand.
...
- Optionally clone the Notify repository
