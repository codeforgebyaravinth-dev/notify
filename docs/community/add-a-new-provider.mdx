...
description: "Contribute a new notification provider to Notify with a step-by-step guide covering package setup, the provider interface, testing, and pull requests."
---

Interested in expanding Notify's capabilities? By contributing to our growing ecosystem, you can enhance Notify's reach and impact.

## How to add a new provider?

Notify currently supports five channels `in_app`, `push`, `email`, `chat` and `sms`.
...
Fork the notify repository and clone it in your local machine.
...
<Steps>
  <Step title="Create credentials config">
    Every provider requires some credentials to create an instance. Notify will add these credentials fields in the integration store provider's form so that users can use their credentials to connect to their preferred provider to use for that channel notification. For example, in the above added `example-provider`, we have only one credential `ApiKey`. We will need to add a config object for `example-provider` with all existing provider's configs like below.
...
<Steps>
  <Step title="Create a provider handler">
    ...
    import { ExampleProviderEmailProvider } from '@notify/providers';
...
Run notify in your local machine. Read [here](/community/run-in-local-machine) to learn on how to run notify on a local machine and test this new provider.
...
Hurray 🎉! You have successfully added a new provider in Notify!
