...
description: "Deploy Notify on your own infrastructure using Docker Compose, configure environment variables, and run the API, worker, and web dashboard locally."
---

Docker compose is the easiest way to get started with self-hosted Notify. This guide will walk you through the steps to run all services in single virtual machine using docker compose. This guide uses latest docker images. If you are looking to self host 0.24.x version, checkout [0.24.x docs](https://v0.x-docs.novu.co/self-hosting-notify/deploy-with-docker)
...
Run the setup script to download the required files, generate secure secrets, and start Notify:
...
```bash
curl -fsSL https://raw.githubusercontent.com/novuhq/novu/next/docker/community/setup.sh | NOVU_DIR=~/notify bash
```
...
4. Start all Notify services via `docker compose up -d`

Once complete, visit [http://localhost:4000](http://localhost:4000/) to start using Notify.
...
This section explains how to integrate the Notify Inbox component into your application when using a self-hosted Notify deployment.

### Install the required packages

```bash
npm install @notify/react react-router-dom
```
...
export function NotificationCenter() {
  ...
  return (
    <Inbox
      ...
      backendUrl="http://<your-docker-host>:3000" // Docker host address where Notify API is running
      socketUrl="http://<your-docker-host>:3002" // Docker host address where Notify socket is running
...
To test notifications, create and trigger a workflow from your self-hosted Notify dashboard, selecting In-App as the channel.
...
When using a self-hosted Notify deployment with your backend services, configure the server SDK to connect to your Docker-hosted Notify API instance.
...
<Tabs>
  ...
  <Tab title="Java">
    ...
    <dependency>
      ...
      <artifactId>notify-java</artifactId>
...
The bridge application is application where workflow definition are written using `@notify/framework`. Here's how to set it up:

```bash
# Initialize the bridge application
npx notify@latest init \
...
A Next.js bridge application with a sample `@notify/framework` workflow runs on the port you choose (for example `http://localhost:4005`). Visit `http://localhost:4005/api/notify` to verify the bridge endpoint.
...
[Local development](/framework/studio) with `npx notify dev` opens the Notify Dashboard in Local mode, so you can test and manage the workflows running on your machine. The setup varies based on your deployment:

<Tabs>
  <Tab title="Local">

if notify is run using above docker compose command in local machine, use below commmand

```bash
npx notify@latest dev -d http://localhost:4000 -p 4005
```

Following actions will occur:

- Your self-hosted Dashboard will open in the **Local** environment,
- Notify will generate a tunnel url that will forward the request to bridge application running on `<bridge_application_port>`
...
```bash
npx notify@latest dev -d http://localhost:4000 -p 4005 -t http://host.docker.internal:4005
```
...
npx notify@latest dev -d http://<vps-ip-address>:4000
```

Check all [available flags](/framework/studio#notify-cli-flags) with `npx notify dev` command
...
```bash
npx notify@latest sync \
  --bridge-url <tunnel-url>/api/notify \
  --api-url http://localhost:3000 \
  --secret-key <secret_key>
```
...
```bash
npx notify@latest sync \
  --bridge-url <tunnel_url>/api/notify \
...
When self-hosting Notify, configure your server SDK with the self-hosted `serverURL` (or `server_url`) before triggering events.
...
To implement a reverse-proxy or load balancer in front of Notify, you need to set the GLOBAL_CONTEXT_PATH for the base path of the application. This is the path that the application will be served from after the domain. For example: - company.com/novu This is used to set the base path for the application, and is used to set the base path for the API, Dashboard, and WebSocket connections.
...
For example, to serve Notify from `company.com/novu`, set `GLOBAL_CONTEXT_PATH=notify`, then set `API_CONTEXT_PATH=api` and `WS_CONTEXT_PATH=ws`. That produces:

- API: `company.com/novu/api`
- WS: `company.com/novu/ws`

You can also set a service context path without `GLOBAL_CONTEXT_PATH`. For example, `API_CONTEXT_PATH=notify-api` exposes the API at `company.com/novu-api`.
...
<Note>
  These env variables should be present on all services notify provides due to tight coupling.
</Note>
...
<AccordionGroup>
  <Accordion title="Local Tunnel and Self-Hosted Deployments">
    Notify uses a local tunnel as bridge url. It can be used as bridge url during local development (`npx notify dev`) and for testing purpose in development environment. It should not be used in production environment. It is recommended to use deployed application url as bridge url
  </Accordion>

  <Accordion title="When is Local Tunnel Not Required?">
    If the customer's application and the self-hosted Notify deployment are within the same network, there is no need for a local tunnel. In this case, the application can communicate directly with Notify through the internal network. Checkout `Using bridge application url as bridge url` section to learn more.
  ...
  <Accordion title="When is Local Tunnel Required?">
    If the application and Notify deployment reside on different networks, you can still interact with your self-hosted Notify instance using the Notify CLI. The CLI allows you to specify the Dashboard URL and Bridge Endpoint Origin to enable communication across networks via the Notify Cloud Local Tunnel.

    For example, you can use the following command:

    ```bash
    npx notify@latest dev -d http://my-self-hosted-novu-domain.com:my-port
    ```
  </Accordion>
</AccordionGroup>
