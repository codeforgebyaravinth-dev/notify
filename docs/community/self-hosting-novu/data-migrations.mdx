...
description: "Run database data migrations for self-hosted Notify deployments to keep MongoDB and Postgres schemas in sync with the latest platform release."
---

On occasion, Notify may introduce features that require changes to the database schema or data. This usually happens when
...
## How to Run Notify Migrations (Manual Process)

Notify does **not automatically run database migrations** when deploying new versions. If you're self-hosting or managing
deployments outside a CI/CD pipeline, follow these steps to safely run migrations against your MongoDB database.

For Notify Cloud, the Notify team have a custom deployment service that runs migrations on AWS and is triggered by our team.
...
For Notify Self-Hosting, each migration can be run locally against your Mongodb using known connection strings that have
...
- **Notify does not use a versioning or idempotent migration strategy.**
...
This table maps each migration script to the likely Notify release version, based on the date the script was added and the
closest following release tag.

| **Release Version**                                              | **Release Date** | **Migrations Introduced**                                                                                                                                                                                                                                                                                                                                                        |
|------------------------------------------------------------------|------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`v0.17.2`](https://github.com/novuhq/novu/releases/tag/v0.17.2) | 2023-08-13       | `notify-integrations` *(Integration Store)*                                                                                                                                                                                                                                                                                                                                        |
...
|                                                              | Integration Store       | `./novu-integrations/notify-integrations.migration.ts`                                                                                      |
...
<Steps>
  <Step title="Clone the Notify Repository">
...
Notify **does not implement a versioning system** or record migration history in the database. This means:
...
#
# This script runs one or more database migration scripts for the Notify project.
# It supports:
...
MIGRATION_MAP_v0_17_2="notify-integrations"
...
get_migration_path() {
  case "$1" in
    ...
    notify-integrations) echo "notify-integrations/notify-integrations.migration.ts" ;;
    ...
    integration-scheme-update) echo "integration-scheme-update/add-primary-priority-migration.ts integration-scheme-update/update-primary-for-disabled-notify-integrations.ts integration-scheme-update/add-integration-identifier-migration.ts" ;;
...
2023-07-31 notify-integrations
...
./novu-integrations/notify-integrations.migration.ts
...
./integration-scheme-update/update-primary-for-disabled-notify-integrations.ts
