...
- If there is any issue in running Notify on a local machine, please share system details like operating system, RAM size, npm version and node version.
- If there is any issue in our backend SDKs, please mention the SDK version and relevant details.
- If looking for self-hosting support, please share `Notify Version` and remote server details. Our latest version is `0.24.0`. Please be patient with self-hosting help. We are a small team and we will try our best to help you.
...
- Power community members are just one step away from becoming a Notify ambassador.

### Become a Notify Ambassador

A Notify ambassador is a trusted power member of the community that can do the following:

- Love exploring and sharing their knowledge of any technology with other developers.
- Often write and speak about Notify.
- Possess strong Notify expertise and often help and support Notify users within the community.
- Socially influence developers with some knowledge of Notify.
- Consistently contribute to Notify OSS product.

We are launching the Notify Ambassador program soon with amazing perks. Stay tuned!
...
- Content writers program: Apply to become a content writer with us and get paid to write about use-cases highlighting how you solved a problem using Notify
- Write an article about what you built with Notify and share it with the community. We'll give you a shout out!
...
<Note>
  At Notify, we believe that no contribution is small, and the only wrong question is the one that
...
- We have frequent (replace with interval - twice weekly, weekly, etc.) and topic-specific office hours that offer the ability to engage in real-time with Notify staff and other community members
...
Together, we'll shape the future of Notify. 🫂
