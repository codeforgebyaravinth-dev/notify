---
title: 'How Notify Works - Architecture and Notification Flow'
sidebarTitle: 'How Notify Works'
description: 'A high-level overview of Notify’s architecture: workflows, triggers, subscribers, and environments, and how they connect to support in-app and external channels.'
---


Notify triggers workflows that deliver notifications to subscribers across in-app, email, SMS, push, and chat channels. Your application sends an event to the Notify API, Notify resolves the subscriber and executes each workflow step, then delivers messages through configured integrations or the [`<Inbox />`](/platform/inbox) component.

## How does the Inbox component fit in the notification lifecycle?

The [`<Inbox />`](/platform/inbox) component is Notify’s in-app delivery endpoint. This component makes it possible for users to receive and manage notifications directly within your application interface.
...
## What are the core concepts in Notify?

At a high level, Notify’s architecture is built around modular but connected concepts. Each concept serves a specific role in the delivery pipeline and is scoped to an environment.

<Frame caption="Notify architecture overview showing organizations, environments, workflows, subscribers, channels, and integrations.">
  ![How Notify works](/images/how-novu-works.png)
</Frame>

### How do organizations and environments work?

Everything starts with your organization, the top-level entity in Notify. Each organization can have multiple environments, such as:
...
A workflow defines the steps Notify takes to deliver a notification. Steps are either channel steps (email, in-app, push, chat, and SMS) or action steps (Delay and Digest).
...
Subscriber resolution is part of workflow execution. Notify uses the subscriber's metadata to determine how messages are personalized, delivered, and stored.
...
Integrations connect Notify to external delivery providers (for example, SendGrid, Twilio, Firebase Cloud Messaging, Slack). They are configured per environment and mapped to specific channels.

During workflow execution, Notify uses these integrations to send messages for steps that require external delivery. The presence of an integration for a given channel is required for a workflow step to succeed.
...
Your application calls Notify’s API and passes in:
 ...
 - An optional `payload` to customize message content for example, `{{firstName}}`, `{{orderId}}`
  ...
  <Step title="Notify resolves the subscriber">
  Notify looks up the subscriber using the ID you provided. This includes:
  ...
  <Step>
  If a step fails (for example, due to a provider error), then Notify retries it without re-executing previous steps. This ensures fault tolerance and message-level traceability.
  </Step>

  <Step title="Messages are delivered">
  Once a step completes, Notify uses the environment’s configured integrations to deliver the message. For example:
  ...
  Each triggered workflow becomes an event with a unique `transactionId`. Notify records:
  - Which steps ran.
...
The Notify pipeline is designed to be consistent, observable, and scalable across channels and environments.
