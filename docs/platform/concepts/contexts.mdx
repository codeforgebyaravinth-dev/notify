---
title: 'How do contexts work in Notify?'
sidebarTitle: 'Contexts'
description: "Learn how contexts work in Notify, how they differ from payloads, and how they scope, personalize, and filter notifications across workflows and Inbox."
---
...
![Notify contexts feature showing tenant-scoped notification isolation in the dashboard](/images/inbox/Contexts.png)

## How contexts fit into Notify's model
...
Contexts are persistent. Unlike a payload, which exists only for a single workflow execution, context data is stored in Notify and reused across triggers, templates, the [Inbox](/platform/inbox), and the Activity Feed.
...
Each key in the `context` object is a context type. Notify supports three formats per key:
...
Contexts are **auto-created** the first time Notify sees them (via a workflow trigger or the Inbox).
...
**Inbox** and other subscriber-facing APIs only find or create contexts and do **not** update existing `data`. You can also manage context data through the [Contexts API](/api-reference/contexts/context-schema) or the Notify dashboard.
...
When a subscriber switches tenants in your app, re-render the Inbox with the new context. Notify refetches notifications and reconnects the WebSocket scope automatically.
