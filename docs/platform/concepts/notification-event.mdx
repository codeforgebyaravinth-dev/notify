...
description: "Understand the Notify notification lifecycle and the key entities - workflows, triggers, subscribers, jobs, and messages - that make up a notification."
---

In Notify, notifications are the core building blocks of your product’s communication experience. When your user receives a message via an in-app alert, an email, or a push notification, what they’re seeing is the final output of a notification.
...
A notification doesn't exist in isolation, it’s the result of a sequence of interconnected entities working together. Here’s how it works in Notify:
...
Something meaningful happens in your application, such as a user signing up, a password reset being requested, or a comment being posted. You emit this event to Notify using the [Event Trigger API](/api-reference/events/trigger-event).
...
This is how your application tells Notify, “It’s time to send something.”

### Workflow

Notify matches the incoming event to a predefined workflow. This workflow is your logic for determining:
...
Once the workflow starts executing, Notify creates a notification for each subscriber involved. This notification acts as the central entity tracking everything that happens - every job, message, and delivery status. It includes:
...
A notification in Notify is more than just a message, it’s a full record of what happened from the moment an event was triggered to when one or more messages were delivered or failed to deliver.
...
These fields are accessible via the [Notifications API](/api-reference/notifications/list-all-events) and are also displayed in the Notify dashboard’s [Activity Feed](https://dashboard.novu.co/activity-feed) for visual traceability.

## Frequently asked questions
These are some of the most frequently asked questions about notifications in Notify.
