---
title: 'Integrations'
description: "Learn how Notify integrations connect your account to third-party email, SMS, push, and chat providers with credentials, environments, and identifiers."
---

In Notify, integrations are configured connections to third-party services that deliver notifications across supported channels, which are email, SMS, push, and chat.

The In-App channel is handled internally by Notify and does not require a third-party integration.

Each integration connects Notify to a specific provider, which workflows use to deliver notifications over the related channel.

## How integrations fit into the notification flow

Workflows in Notify handle the logic of what message to send and when. Integrations handle how that message is delivered by routing the message through a specific provider.

When a workflow step executes a channel-based action (such as sending an email), Notify:
...
You can have multiple integrations for the same provider, for example, using two different SendGrid accounts for staging and production environments. Notify allows you to manage these as separate, named integrations.

Refer to this resource to see the full list of [providerIds used in Notify](https://github.com/novuhq/novu/blob/next/packages/framework/src/shared.ts#L103).

<Note>
  Notify also lets you use [Trigger Overrides](/platform/integrations/trigger-overrides) to modify the default behavior of a message during workflow trigger, such as overriding the notification title or content, or using a different integration than the primary integration. This works alongside integrations to fine-tune delivery behavior. 
</Note>
...
Integrations require credentials to authenticate with third-party providers. These credentials are encrypted at rest and managed securely within Notify. 
...
Notify supports a wide range of providers across different channels:
