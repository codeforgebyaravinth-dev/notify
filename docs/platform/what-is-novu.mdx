...
Notify is open-source communication infrastructure for products that need to reach users reliably and hold conversations with them. It gives you two complementary capabilities on one platform:
...
Notify is designed to fit and scale with your existing system. Trusted by startups and enterprises for its developer-first design and cost-effective scalability, it delivers notifications and agent replies reliably without unnecessary complexity.
...
## Why use Notify for notification infrastructure?

Notify is designed to provide a powerful, flexible, and easy-to-use communication solution that helps teams save time, reduce complexity, and improve user engagement across multiple platforms. Here's what sets it apart from other notification and agent infrastructure solutions:

### The only fully open-source notification infrastructure

- **Backed by a growing open-source community**. Notify is 100% open-source, with a rich community of creative minds constantly contributing to improve its capabilities.
- **Built with an API-first approach**. Developers can deeply customize workflows and automate notifications through Notify APIs.
- **Easily integrates with existing tools**. Notify works with modern development stacks, enabling smooth adoption without disrupting workflows.
...
- **Optimizes delivery strategies**. Notify enables multi-step workflows, ensuring users receive messages in the most effective channel.
...
- **No-code workflow automation**. The drag-and-drop workflow builder makes notification automation accessible to your non-technical teams via the Notify's dashboard.
- **Provides ultimate control with code-first workflows**. [The Notify Framework SDK](/platform/sdks) lets developers define workflows in code, integrate with CI/CD, and add custom automation logic.
...
- **Extends functionality with the Notify Framework SDK**. Developers can integrate custom workflows beyond Notify's default capabilities.
...
Transactional workflows and agent conversations share the same Notify account, subscribers, and channels. ACI extends the workflow system; it does not replace it.
...
<Columns cols={2}>
  ...
  <Card icon="brain-circuit" href="/agents/get-started/mental-model" title="Mental model">
    See how inbound messages flow from a channel through Notify to your agent and back.
  </Card>
</Columns>
...
Workflows can send messages on behalf of an agent across email, chat, and other channels. When a user replies to that message, Notify can open a new agent conversation and continue the thread in context.
...
Here is how developers, startups, and enterprises are currently using Notify in real-world projects:
...
- Show unread notifications directly in the app using Notify's Inbox component.
...
- Delay and schedule notifications using Notify's workflow automation.
...
- Manage user preferences with Notify's built-in preference center.
...
<AccordionGroup>
  <Accordion title="What makes Notify different from other notification platforms?">
    Notify is fully open source and unifies transactional notifications and agent conversations on one platform. It supports no-code workflows, code-first workflows with the Notify Framework, a drop-in Inbox component, and Agent Communication Infrastructure (ACI) for multi-channel agents.
  </Accordion>
  <Accordion title="Can I self-host Notify?">
    Yes. Notify can be self-hosted for full control over your notification infrastructure. See the [self-hosting guide](/community/self-hosting-notify/overview) for deployment options.
  </Accordion>
  <Accordion title="Which channels can I send notifications through?">
    Notify supports in-app (Inbox), email, SMS, push, and chat. Connect your preferred providers through the [Integrations store](/platform/integrations).
  </Accordion>
</AccordionGroup>
