---
title: 'Translations'
description: "Translate Notify workflow step content into multiple languages with translation keys, locales, and per-subscriber locale resolution at trigger time."
---

Notify supports multi-language translations for workflows, allowing notifications to dynamically adapt to each of your subscriber's preferred language. You can enable, configure, and manage translations across your notification workflows from the Notify dashboard.

<Note>
  The translations feature is in `beta` and available on `Team` and `Enterprise` [plans](https://novu.co/pricing).
</Note>

Each workflow can support multiple locales. Notify automatically selects the correct translation based on the subscriber’s locale.

<Warning>
  Notify currently supports locale codes in `language_REGION` format only, for example, `en_US`, `fr_FR`. Language-only locale codes such as `en` are not supported.
</Warning>
...
Manage translations from the [translation page](https://dashboard.novu.co/translations) in the Notify dashboard. Set a default language and add target languages as needed.
...
If you prefer to create/update translations in a JSON file outside the Notify dashboard, you can do so by exporting and importing them.
...
<Note>
  Translation files must be named using the `language_REGION.json` format, for example, `en_US.json`, `fr_FR.json`. Notify detects the locale automatically using the file name.
</Note>
...
The Notify CLI can download translation files from Notify Cloud to your machine, or upload local JSON files back to your workspace. This is useful for version control, bulk edits, and CI workflows.

Use `npx notify` (or a globally installed `notify` binary) with your Notify secret key and API URL to pull or push translations.

### Pull translations

`notify translations pull` downloads all translation files from Notify Cloud into a local directory (default: `./translations`).

```bash
# Pull to the default directory (./translations)
npx notify translations pull -s YOUR_SECRET_KEY

# Pull to a custom directory
npx notify translations pull -s YOUR_SECRET_KEY -d ./my-translations

# Use the EU API endpoint
npx notify translations pull -s YOUR_SECRET_KEY -a https://eu.api.novu.co
```

| Option | Description |
| --- | --- |
| `-s, --secret-key <key>` | Your Notify secret key (required unless set via env) |
| `-a, --api-url <url>` | Notify API base URL (default: `https://api.novu.co`) |
| `-d, --directory <path>` | Output directory (default: `./translations`) |

### Push translations

`notify translations push` uploads translation JSON files from a local directory to Notify Cloud (default directory: `./translations`).

```bash
# Push from the default directory (./translations)
npx notify translations push -s YOUR_SECRET_KEY

# Push from a custom directory
npx notify translations push -s YOUR_SECRET_KEY -d ./my-translations

# Use the EU API endpoint
npx notify translations push -s YOUR_SECRET_KEY -a https://eu.api.novu.co
```
...
npx notify translations pull
npx notify translations push
```
...
- Run **pull** once to confirm the on-disk layout matches what Notify expects before automating in CI.

<Warning>
  Never commit Notify secret keys. Use environment variables or your CI provider’s secrets store for `NOVU_SECRET_KEY`.
</Warning>
...
- Ensure subscriber locales are mapped to a supported `language_REGION` format before sending them to Notify.
...
<AccordionGroup>
  <Accordion title="Is translation resolved when the notification is fetched, or when it is sent?">
    ...
    Once the step has executed, the translated in-app notification content is stored with the message. Notify does not re-translate existing in-app notifications when they are fetched. Historical messages keep the locale they were created with.
  </Accordion>
</AccordionGroup>
