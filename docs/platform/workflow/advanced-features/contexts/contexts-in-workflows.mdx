...
description: "Apply contexts in Notify workflows to personalize notification templates, control workflow logic, and customize per-tenant behavior in the Inbox."
---
...
Once a context is created either through the Notify dashboard or API, its data becomes available for use in your templates editors, step conditions, and for customizing the [`<Inbox />`](/platform/inbox/configuration/inbox-with-context) component.
...
Once you start using contexts in your workflows, Notify provides full observability so you can monitor and debug your context usage after a workflow has been triggered.

### Searching for workflow runs

You can filter your workflow runs to find all executions associated with a specific context.

1. Navigate to the **Activity Feed** in your Notify dashboard.
...
To confirm that Notify received and processed your context data correctly, you can inspect the API traces for a specific run.
