...
description: "Create, update, retrieve, and delete workflow contexts in Notify through the dashboard or the REST API to power per-tenant and per-environment configuration."
---

Notify lets you manage contexts through the Notify dashboard, or the Notify API. This lets you create, view, update, and delete context entities to suit your application's needs.
...
When defining contexts, Notify supports multiple formats per key-value pair that let you store and reference metadata relevant to your workflows and templates.
...
You can create a new context via the Notify dashboard or API when you want to register reusable metadata. After creation, this context becomes available to all workflows and templates within your environment.
...
<Steps>
  ...
  <Step title="Open Contexts">
    In the Notify dashboard sidebar, click **Contexts**.
  </Step>
...
Notify provides an API to create a context. If a context with the same `type:id` combination already exists, then the request will fail.
...
Contexts can also be created automatically when you trigger a workflow that includes a new context object. If the specified `type:id` doesn't exist, then Notify automatically creates it before running the workflow.
...
If a matching context already exists and you pass only a string identifier, Notify reuses it without changing stored `data`. If you pass a rich object with a `data` field, Notify replaces the stored `data` with the values from your trigger request. This upsert behavior applies to workflow triggers only.
...
Notify provides an API to update an existing context. The `data` object is replaced entirely during updates (not merged). Include all fields you want to retain.
...
Notify provides an API to retrieve a single, specific context by providing its `type` and `id` in the URL.
...
Notify provides an API that lists or searches available contexts. Use pagination and search parameters to retrieve subsets efficiently.
...
Delete a context if it's no longer needed. This action permanently removes the context from your Notify environment. 
...
Notify provides an API that you can use to delete a context. Once deleted, the context will no longer be available for use in new workflow executions.
