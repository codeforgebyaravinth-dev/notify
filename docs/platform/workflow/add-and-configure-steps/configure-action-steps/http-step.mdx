---
title: 'HTTP Step'
description: "Use the HTTP step in Notify workflows to call external APIs, enrich payloads with response data, and coordinate side effects with the rest of your workflow."
---

The HTTP step is an Action step that allows workflows to call external APIs during execution. This provides a native way to interact with systems outside Notify directly from your workflows. It is useful for tasks such as enriching data, enabling conditional branching based on external responses, or communicating with internal delivery systems.
...
With each API request, notify send `notify-signature` header. This header is a HMAC signature of the request body and the timestamp. You can verify the authenticity of the request by verifying the HMAC signature. Below example shows how to verify the HMAC signature in a Next.js API route. Here, Notify secret key is used to generate the HMAC signature. You can copy the secret key from the [Notify API Keys](https://dashboard.novu.co/api-keys) page.

<Note>
  Replace `YOUR_SUBSCRIBER_ID` with a subscriber from your Notify dashboard and store your secret key as `NOVU_SECRET_KEY` in environment variables.
</Note>

<Prompt description="Implement a Notify HTTP webhook handler" icon="zap" actions={["copy", "cursor"]}>
I need to implement an HTTP endpoint that will be called by Notify's notification workflow engine.
...
Headers:
  ...
  notify-signature: t=TIMESTAMP,v1=HMAC_SHA256_SIGNATURE
...
Every request from Notify includes a `notify-signature` header to prove authenticity.
...
3. Compute HMAC-SHA256 of that string using your Notify secret key
...
2. Reads the raw request body and verifies the `notify-signature` header
...
```typescript title="api/notify-http-step-request/route.ts"
...
export async function POST(req: NextRequest) {
  ...
  const parts = headers["notify-signature"].split(",");
