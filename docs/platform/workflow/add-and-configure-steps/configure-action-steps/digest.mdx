---
title: 'Digest Step'
description: "Batch multiple Notify workflow trigger events into a single execution with the digest step using configurable time windows and grouping rules."
---


After adding a Digest step to a workflow, Notify batches multiple trigger events and executes downstream steps only once per digest window, instead of once per event.
