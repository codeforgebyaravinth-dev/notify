---
title: 'Code Steps'
description: 'Learn how to add code steps to a workflow, publish them with the Notify CLI, and use code-managed steps with UI-managed steps in a workflow.'
---

Notify allows you to write step handlers as TypeScript code in your project and publish them to Notify as serverless functions. Instead of configuring notification content in a visual editor, you define a handler file that returns the output for your step. Notify calls it at send time with the subscriber data, trigger payload, and any dashboard-defined controls.
...
<Steps>
  ...
  <Step title="Get the CLI command from the dashboard">
    The Notify dashboard shows a prefilled publish command in the step editor screen, click on the **Copy** button to copy the command. It includes your secret key, workflow ID, step ID, and API URL.
  </Step>

  <Step title="Run the CLI command in your project">
    ```bash
    npx notify step publish \
    ...
    | --- | --- | --- | --- |
    | `-s, --secret-key <key>` | string | `$NOVU_SECRET_KEY` | Notify API secret key |
    | `-a, --api-url <url>` | string | config or `https://api.novu.co` | Notify API URL |
    ...
    1. Look up the step type from the Notify API.
    2. Auto-scaffold a placeholder handler at `notify/<workflowId>/<stepId>.step.tsx`.
    3. Bundle all handlers in the `notify/` directory.
    4. Deploy to Notify.
  ...
  <Step title="Edit the handler">
    The generated file will be located at `notify/<workflowId>/<stepId>.step.tsx`. The folder name is the `workflowId`. The CLI reads it from the path at publish time. You should commit this file to your repository.

    Open the generated file and replace the placeholder content with your real logic. Each handler receives the full context:

    ```typescript title="notify/workflow-id/step-id.step.tsx"
  ...
  <Step title="Republish when you make changes">
    ...
    If the bundle content changed, Notify deploys a new version. This `publish` command by default will publish all step handler in notify folder, if you want to publish just one workflow or one step you can specify `--workflow` and `--step` flags
  </Step>
</Steps>
...
```typescript
import { step } from '@notify/framework/step-resolver';
import { z } from 'zod';
...
Provider overrides let you customize the raw request sent to the underlying notification provider for a step. This is useful for setting provider-specific options not exposed by Notify standard output schema.
...
Notify sanitizes HTML in `email` and `in_app` outputs by default to prevent XSS vulnerabilities. To opt out, you can set `disableOutputSanitization`:
...
You can install `@notify/framework` as a dev dependency to get full TypeScript types for `controls`, `payload`, `subscriber`, `context`, and `steps`:

```bash
npm install --save-dev @notify/framework
```

<Note>
  The CLI publishes to non production environments only. Publishing directly to Production is blocked. To promote changes to Production, use the **Publish changes** button in the Notify dashboard.
</Note>
...
| "Workflow not found" / "Step not found" | Create the workflow and step in the Notify dashboard before publishing. |
