...
You can use data from the [payload schema](/platform/workflow/configure-workflow#payload-schema) or custom payload data passed during the workflow trigger call to the Notify API to configure step conditions. This allows you to define dynamic rules based on the data unique to each workflow execution.
...
<Note>
  **Advanced application state calculations**
  ...
  For more complex scenarios where you need to perform advanced calculations based on your application state, you can use the [Notify Framework](/framework/typescript/steps).
</Note>