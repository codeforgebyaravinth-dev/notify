...
description: 'Learn how workflow steps work in Notify, the different step types available, and how to add and execute steps in a notification workflow.'
---
...
A workflow is built as a sequence of steps, executed linearly from top to bottom. You define and arrange steps using the workflow editor in the Notify dashboard, where each node on the canvas represents a single step in the execution flow.

Each step operates independently. If one step fails, other steps in the workflow can still succeed. Workflows support two types of steps from the Notify dashboard.

![Types of steps](/images/workflows/add-and-configure-steps/steps.png)

<Note>
  You can create a custom step using Notify Framework. For more information, refer to the [Custom steps](/framework/custom).
</Note>
...
<Columns cols={2}>
  ...
  <Card
    ...
    icon="bell"
    href="/platform/inbox"
    description="Deliver notifications directly inside your application through the Notify <Inbox/>."
  />
  <Card
    title="Push"
    icon="smartphone"
...
<Note>
  Notify provides built-in [demo integrations](/platform/integrations/demo-integration) for the Email and SMS channels. These sandboxed providers let you test workflows before configuring a provider.
</Note>
...
The Throttle step lets you limit the number of workflow executions within a specified time window. You can configure throttling directly in Notify workflow editor. By setting limits, you can prevent subscribers from receiving duplicate or excessive notifications when a trigger fires repeatedly.
...
The HTTP step allows workflows to call external APIs during execution. It provides a native way to interact with systems outside of Notify directly from your workflows without relying on external webhooks or custom infrastructure.
