...
description: 'Learn how to personalize notification content in Notify using template variables, context data, and LiquidJS filters across all channel template editors.'
---


Notify allows you to personalize notification content in all channel template editors using variables, conditional logic, and runtime data. This enables you to generate notifications that adapt to each subscriber and reflect the data available at the time a workflow is executed.
...
Context variables come from context data. Once a context is created either through the Notify dashboard or API, its data becomes available in all channel template editors and can be referenced using the context namespace.

```liquid
{{ context.* }}
```

Context variables are typically used for tenant details, branding and configuration that should not live in the workflow payload or data shared amongst different entities in Notify.
...
All Notify template editors support [LiquidJS](https://liquidjs.com/index.html). This allows you to add logic and control flow to your notification content, including conditional rendering, iteration, and value transformation directly inside channel template editors.
...
Notify provides a built-in variable configuration UI in the template editor that allows you to apply LiquidJS filters to variable output.
