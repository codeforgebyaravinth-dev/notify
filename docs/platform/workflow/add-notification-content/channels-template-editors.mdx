...
description: "Design and configure notification content for email, in-app, push, and SMS channels using the Notify template editors in the dashboard."
---
...
The in-app template editor defines how notifications appear inside your application through the Notify [`<Inbox />`](/platform/inbox). In-app notifications are UI-driven, not just message-driven. This means that the template is responsible for both content and interaction.
...
By default, Notify sanitizes both the subject and body fields to prevent unsafe HTML from being rendered. This includes sanitizing:
...
The email template editor defines how email notifications are structured, rendered, and delivered. Notify dashboard provides multiple authoring modes:

<Note>
  You can submit custom HTML email templates via the Notify API. This method supports workflows where you render templates externally and use Notify purely for delivery.
</Note>
...
When the workflow runs, Notify evaluates the array you provide and repeats the block’s content for every element, allowing you to build dynamic lists.

<iframe
  width="100%"
  height="400"
  src="https://www.youtube.com/embed/yC--z0mQ3Bg"
  title="Learn how to use Notify email editor Repeat block"
  frameBorder="0"
...
- Notify automatically inserts a Repeat block that iterates over the digested events.
...
The code editor provides a built-in interface for writing raw HTML and embedding LiquidJS expressions directly inside the Notify dashboard. This gives you full control over layout, structure, and rendering beyond the capabilities of Notify's block editor.
...
![Notify code editor](/images/workflows/add-notification-content/channels-template-editors/code-editor.png)
...
<Note>
  On the Free plan, only one layout can be created per environment. Pro and higher plans support up to 100 layouts per environment. If you need more than 100 layouts, please [contact Support](mailto:support@notify.co).
</Note>
...
1. Navigate to [Workflows page](https://dashboard.novu.co/workflows) on Notify dashboard.
...
Email layouts are managed from the **Layouts** section of the Notify dashboard. From here, you can create, edit, delete, and reuse layouts across workflows.
...
1. Navigate to the [Layout page](https://dashboard.novu.co/layouts) on Notify dashboard.
2. Click on the **Create layout** button on the top right corner.
3. Fill in:
  - **Layout name**: Provide a human-readable name.
  - **Identifier**: Provide a unique ID. Notify generates one automatically if you do not provide it. You cannot change the identifier after it is created.
...
1. Navigate to [Layout page](https://dashboard.novu.co/layouts) on Notify dashboard.
