...
description: "Create a Notify workflow in the dashboard and define its identifier, metadata, tags, and initial configuration to start orchestrating multi-channel notifications."
---

A workflow defines how Notify delivers notifications for a specific event. It contains the steps, templates, and rules that control how messages are sent across channels.

You can create workflows in the Notify dashboard, using the [Notify API](/api-reference/workflows/create-a-workflow) or define them in code using the [Notify Framework](/framework/introduction#create-a-workflow). This guide focuses on creating workflows in the Notify dashboard.
...
You can create a Notify workflow in the following ways:
...
<Steps>
  <Step title="Go to the Notify Dashboard">
    Sign in at the [Notify Dashboard](https://dashboard.novu.co).
  </Step>
...
<Warning>
  Deleting a workflow is permanent and cannot be undone. Any existing triggers referencing the deleted workflow will fail with a `workflow not found` error. Execution history is preserved in the [Notify Activity Feed](/platform/workflow/monitor-and-debug-workflow).
</Warning>
