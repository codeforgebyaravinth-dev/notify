...
description: "Learn how roles and permissions work in Notify organizations. Assign admin, developer, or viewer roles to control who can edit workflows and integrations."
---
...
Notify uses a role-based access control (RBAC) model at the [organization level](/platform/account/organizations). Each member in the organization is assigned a role that determines the actions they can perform within the Notify dashboard. Every user can belong to more than one organization, each with separate configurations and permissions.
...
These permissions are valid only on the actions performed on the Notify dashboard. Secret Keys are scoped to the whole environment and not to a specific role. If a viewer role member has access to the secret key that member can perform all possible actions on the environment.
...
On Notify Cloud, every member can open **Settings → Billing**, but only **Owners** can view plans, change subscriptions, and manage payment methods. Other roles see a restricted-state message with links to team settings and this permissions matrix.
