...
description: 'Sign in to Notify Cloud with Google, GitHub, email and password, or enterprise OIDC and SAML. Learn about MFA, sessions, and account security.'
---

<Note>
  Dashboard authentication described here applies to [Notify Cloud](https://dashboard.novu.co). Self-hosted deployments use email and password only. See [Self-Hosted and Notify Cloud](/community/self-hosted-and-notify-cloud) for a full comparison.
</Note>

Notify Cloud handles sign-in, session management, and account security for the dashboard. After you authenticate, you can access every [organization](/platform/account/organizations) you belong to and switch between them without signing in again.

## Sign-in methods

Notify Cloud supports the following ways to create an account and sign in:

### Email and password

Create an account with your email address and a password. Notify sends a verification email before your account is fully activated. Use this method if you prefer a traditional credential-based sign-in or if your organization does not use SSO.

### Google and GitHub

Sign up or sign in with your existing **Google** or **GitHub** account. Social sign-in links your Notify account to the provider you choose, so you do not need a separate password.
...
Add a second verification step to protect your account. Notify supports:
...
Users can enable MFA from their account security settings. Enterprise customers can require MFA for all organization members - contact [support@notify.co](mailto:support@notify.co) to configure organization-wide MFA policies.

## Session management

Notify manages authenticated sessions with industry-standard security practices:
