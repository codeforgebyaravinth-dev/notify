---
title: "@notify/react"
description: "API reference for the @notify/react SDK components, including Inbox, Bell, Notifications, Subscription, and hooks for building custom notification experiences."
---

## Requirements

`@notify/react` requires **React 18.0.0 or later** (`^18.0.0` or `^19.0.0`). React 17 and earlier versions are not supported. When rendering Inbox components, you also need `react-dom` at a matching version.

## Components

The @notify/react package provides React components for building notification UIs.
...
```tsx
import { Notifications } from "@notify/react";
...
```tsx
import { InboxContent } from "@notify/react";
