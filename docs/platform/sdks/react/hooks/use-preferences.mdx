...
The Preference type from `@notify/react` includes these properties:
...
```tsx
import type { Preference, ChannelType } from "@notify/react";
import { usePreferences } from "@notify/react";
...
```tsx
import { NotifyProvider, usePreferences, SeverityLevelEnum } from "@notify/react";
