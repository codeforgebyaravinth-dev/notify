---
title: 'Telegram'
description: "Use the Telegram provider in Notify for chat notifications and two-way agent conversations. Create a bot, store the token, and route messages through workflows."
---

The Telegram chat integration lets your application send messages to subscribers through a Telegram bot. Each subscriber connects their Telegram chat to Notify once; after that, Notify can deliver workflow notifications or agent replies to that chat.
...
Every Telegram integration uses one bot. Create it with BotFather before adding the provider in Notify.
...
<Steps>
  ...
  <Step title="Copy the confirmation message">
    ...
    Copy the full message - Notify can extract the token automatically when you paste it into the dashboard.
  </Step>
</Steps>

<Warning>
  Treat the bot token like a password. Store it only in Notify or on your server. Never embed it in client-side code.
</Warning>

## Configure Telegram in Notify
...
Before Notify can deliver messages, each subscriber must connect their Telegram chat.

### Connection link (recommended)

Notify gives you a link that opens your bot in Telegram. When the subscriber taps **Start**, their chat is linked to their Notify subscriber profile.
...
#### From your app with `@notify/react`

Use `TelegramConnectButton` inside `NotifyProvider`. It opens Telegram for the subscriber and confirms when the chat is connected. This works for any Telegram integration - the integration does not need to be attached to an agent.
...
function ConnectTelegram() {
  return (
    <NotifyProvider subscriberId="user-123" applicationIdentifier="your-app-id">
...
<Steps>
  <Step title="Add a Chat step">
    In the Notify dashboard, open your workflow, add a step, and select **Chat** as the channel.
  </Step>
...
Notify finds the subscriber's linked Telegram chat, renders the message, and sends it through your bot. Verify delivery in **Activity** or in Telegram.
...
<Steps>
  <Step title="Create or open an agent">
    In the Notify dashboard, go to **Agents** and create an agent or open an existing one.
  </Step>

  <Step title="Add Telegram as a provider">
    Add **Telegram** as a provider. Notify creates the integration and links it to your agent.
  </Step>
...
| **Workflow notification** | You trigger a workflow with a Chat step and Notify sends a one-way message to the subscriber's linked chat. |
