...
Before connecting LINE in Notify, create a Messaging API channel in the LINE Developers Console and issue a Channel Access Token.
...
<Steps>
  ...
  <Step title="Issue a Channel Access Token">
    Open your channel, go to the **Messaging API** tab, and issue a **Channel access token** (long-lived). Copy the token - you will paste it into Notify.
  </Step>

  <Step title="Enable webhooks (recommended)">
    On the same tab, enable **Use webhook** if you plan to capture subscriber LINE user IDs from follow or message events. Set the webhook URL to your backend endpoint that creates `line_user` channel endpoints in Notify.
...
## Configure LINE integration in Notify
...
Each subscriber needs a LINE user ID (for example, `U1234567890abcdef`) so Notify knows where to deliver messages. Create a `line_user` channel endpoint for the subscriber using the [channel endpoints API](/api-reference/channel-endpoints/create-a-channel-endpoint).
...
Pass `flex`, `image`, or `sticker` under `overrides.chat` when triggering a workflow. When present, Notify sends that message type instead of plain text.
