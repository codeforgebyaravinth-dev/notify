---
title: "MS Teams Chat Integration with Notify"
sidebarTitle: "MS Teams"
description: "Connect Microsoft Teams to Notify to send chat notifications through workflows. Create an incoming webhook in Teams and store the URL as a Notify credential."
---

The Notify Microsoft Teams integration enables you to send notifications to Teams channels and direct messages (DMs) across different customer workspaces using a single multi-tenant bot.
...
Once a customer connects their workspace, Notify establishes a secure channel connection using your bot's credentials. You then map specific destinations, whether the destinations involve public channels or individual users to channel endpoints, allowing Notify to route notifications dynamically to the correct tenant and conversation context.
...
- Access to [Notify dashboard](https://dashboard.novu.co)

## Azure and Teams configuration

Before you can configure Notify, you must create the infrastructure that hosts your bot. This involves two distinct portals:
...
<Step title="Note the IDs">
    After creating the app, note the Application (client) ID and Directory (tenant) ID. You need these values to configure Microsoft Teams in Notify.
  </Step>
</Steps>
...
<Step title="Copy the secret value">
    Copy the value. This value becomes your `BOT_APP_SECRET`, and you’ll paste it into Notify. You can view the secret only once right after you create it, so save it before you leave the page.
  </Step>
</Steps>

### Add Notify’s redirect URI

When a tenant administrator accepts admin consent, Microsoft redirects the browser to Notify at this URL. Register it in **Azure** on your app registration during setup - before any organization connects Teams.

This is not the optional **Redirect URL** on the Notify integration. That Notify field only controls where the administrator is sent in **your application** after Notify finishes processing the callback.
...
<Step title="Add redirect URI">
    Add the redirect URI shown in the Notify integration settings.
  </Step>
...
<Step title="Set the redirect URI">
    In the **Redirect URI** field, set the redirect URI to the Notify OAuth callback URL:
      - US region
      ```bash
...
Notify also requests `openid` and `profile` during the link-user OAuth flow; Azure includes these automatically for sign-in. Application permissions (above) cover tenant connect and channel messaging. Delegated `User.Read` is only needed for the separate link-user flow.

<Steps>
  <Step title="Grant admin consent in your home tenant">
    On the **API permissions** page, click **Grant admin consent for [your tenant]** and confirm. This applies the application permissions in the tenant where you registered the app. Each organization that connects Teams later grants consent separately through the Notify OAuth flow.
  </Step>
</Steps>
...
## Configure the Teams (Bot) integration in Notify

Now that you’ve configured your Azure Bot, provide Notify with the credentials to integrate it.
...
<Step title="Enter credentials">
    ...
    - **Redirect URL** (Optional): Where the **tenant administrator** is sent in **your application** after Notify processes the OAuth callback. This is not the Azure redirect URI - see [Add Notify's redirect URI](#add-novus-redirect-uri).
    ![Notify integration](/images/channels-and-providers/chat/msteams/notify-msteams-integration.png)
  </Step>
...
From now on, Notify can send messages as your bot once an organization's tenant is connected and the app is installed in Teams.
...
<Note>
  **Tenant connect and user linking are separate steps.** Admin consent connects the organization's Microsoft 365 tenant. Linking a Notify **subscriber** for direct messages is a separate OAuth flow - see [Link a subscriber for direct messages](#link-a-subscriber-for-direct-messages).
</Note>
...
Call `POST /v1/integrations/channel-connections/oauth` with the same body if you are not using the TypeScript SDK. Authenticate with your Notify **secret key**.

Notify returns a URL pointing to Microsoft's administrator consent endpoint (`login.microsoftonline.com/organizations/v2.0/adminconsent`). It includes your Client ID, redirect URI, and the `https://graph.microsoft.com/.default` scope.

For tenant-wide admin consent, set **`autoLinkUser: false`** (or omit `autoLinkUser` - the API treats omitted as `false`). Only when `autoLinkUser` is explicitly `true` does Notify chain a second OAuth flow after admin consent to link that subscriber for DMs.

<Note>
  **`@notify/react` `MsTeamsConnectButton`** defaults `autoLinkUser` to `true` in subscriber mode. That is intended for in-app end-user connect flows. For server-side tenant admin consent (Java, REST, or `@notify/api`), pass `autoLinkUser: false` explicitly.
</Note>
...
The tenant administrator is someone at the **connecting organization** (not a Notify dashboard user). They grant consent in Microsoft - they do not need a Notify account.
...
Microsoft redirects to Notify with `admin_consent=True` and the organization's `tenant` ID. Notify stores the tenant on a channel connection and the flow is complete. You do not handle the callback yourself.

If you set an optional **Redirect URL** on the MS Teams integration in the Notify dashboard, the administrator is sent there after success; otherwise the consent window shows a success message and can be closed.
...
## Tell Notify where to send the messages

Decide where notifications should land in Teams, collect the required IDs, and register them as channel endpoints in Notify.
...
To send a notification to a specific channel, you must discover the Team ID and Channel ID from Microsoft, and then register them in Notify.
...
The `SUBSCRIBER_TENANT_ID` represents the customer's tenant ID, which Notify stored on the `ChannelConnection` object after completing the Admin Consent flow.
...
#### Register the channel endpoint (Notify)

Once you have the IDs, create an `ms_teams_channel` endpoint in Notify. This maps a subscriber to that specific channel.
...
Notify stores this as `ChannelEndpoint<'ms_teams_channel'>`.
...
- The subscriber exists in Notify.
...
This opens a Microsoft sign-in flow with delegated scopes (`openid`, `profile`, `User.Read`). Notify reads the user's identity from the token, installs the bot for that user when possible, and creates an `ms_teams_user` channel endpoint.
...
#### Register the user endpoint (Notify)

Once you have the IDs, create the endpoint in Notify using the `ms_teams_user` type.
...
Notify stores this as `ChannelEndpoint<'ms_teams_user'>`. From here, any workflow that resolves to this endpoint can send a DM from your bot to that user.
...
### Register the webhook endpoint (Notify)
...
<Steps>
  <Step title="Notify sends HTTP POST">
    Notify sends a standard HTTP POST payload to the Teams Workflow URL.
  </Step>
...
| **Workflow notification** | You trigger a workflow with a Chat step and Notify sends a one-way message via your Teams integration or workflow webhook. |
