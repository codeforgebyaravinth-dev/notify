---
title: "Zulip Chat Integration with Notify"
sidebarTitle: 'Zulip'
description: "Connect Zulip to Notify to send chat notifications through workflows. Create a bot in Zulip, generate the webhook URL, and store credentials for stream delivery."
---
...
2. Update the subscriber credentials with this URL using the Zulip provider id. You can do this step by using the `@notify/api` library, as shown below:
...
- The third parameter is the credentials object, in this case, we use the `webhookUrl` property to specify the Zulip channel webhook URL or by calling the `Update Subscriber Credentials` endpoint on Notify API. Check endpoint details [here](/api-reference/subscribers/update-provider-credentials).

3. You are all set up and ready to send your first chat message via our `@notify/api package or directly using the REST API.
