---
title: "WhatsApp Business Chat Integration with Notify"
sidebarTitle: "WhatsApp Business"
description: "Connect WhatsApp Business to Notify to send chat notifications through workflows. Configure phone number IDs, access tokens, and approved message templates."
---
...
To integrate WhatsApp Business with Notify, You will have to create a facebook developer app and obtain the necessary credentials.
...
Copy the following pieces and paste them in the Notify WhatsApp Business integration settings:
...
<Step title="Add a test phone number">
  ...
  This number can be used to test your integration with Notify before submitting for a review.
</Step>

<Step title="Send a test notification from Notify">
...
<Steps>
  <Step title="Open the Chat step">
    In the [Notify Dashboard](https://dashboard.novu.co), open your workflow and select the Chat step that uses the WhatsApp Business integration.
  </Step>
...
Override values may contain [Liquid](/platform/workflow/add-notification-content/personalize-content#apply-logic-with-liquidjs) templates, which Notify compiles at send time:
...
For text messages, `text.body` is the primary content field. If your override omits it, Notify fills it with the rendered Chat step body. Recipient addressing (`to`) is resolved from the subscriber endpoint — prefer addressing a different subscriber when you want a different destination.
...
| **Workflow notification** | You trigger a workflow with a Chat step and Notify sends a one-way template message to the subscriber. |
