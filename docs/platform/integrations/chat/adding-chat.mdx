---
title: "Adding Chat Channel Chat Integration with Notify"
sidebarTitle: "Adding Chat Channel"
description: "Add a chat channel provider in Notify to send chat notifications through workflows. Walks through selecting a provider, storing credentials, and testing delivery."
---
...
<Steps>
<Step title="Enable Chat Channel in Notify Dashboard">
...
- Go to the Notify Dashboard and click **"Integrations"** on the left sidebar
...
<Step title="Setup workflow with chat channel in Notify Dashboard">

- Go to the Notify Dashboard and click **"Workflows"** on the left sidebar
...
Notify's server-side SDKs make integrating Notify's REST APIs straightforward, letting you focus on implementing workflows without dealing with repetitive code.
...
- Go to the Notify Dashboard, navigate to the **"Workflows"** section, and locate your configured workflow
- Click **"Test Workflow"** and provide sample data, such as a phone number or dynamic variables
- Verify delivery in the Notify Logs or your Chat provider's dashboard
...
Add a chat provider integration to your Notify account from the integration store. Enter credentials if required and save the integration. Follow corresponding provider documentation to get the required credentials.
</Step>
