---
title: "Slack Chat Integration with Notify"
sidebarTitle: 'Slack'
description: "Connect Slack to Notify to send workflow chat notifications through OAuth, deliver messages to channels or DMs, and format content with Block Kit."
---

<Note>
  This feature is currently in public beta, please contact us at support@notify.co to enable it for your organization.
</Note>

The Slack chat integration lets your application send notifications directly to your subscribers' Slack workspaces using their own Slack accounts and workspace permissions. With this integration, Notify can deliver messages to Slack channels, direct messages (DMs) to Slack users, and incoming webhooks. For incoming webhooks, Notify can use Slack's native channel picker during OAuth.

Notify handles the full lifecycle of Slack connections and message delivery. You define where notifications should be delivered, and Notify automatically routes each message to the correct Slack workspace, channel, or user.
...
Before integrating Slack chat with Notify, you must create and configure a Slack app. The Slack app manages the OAuth permissions, bot token scopes, and redirect URLs needed for Notify to connect to your users' workspaces securely.

### Create a Slack app
First, you need to create a Slack app. This provides you with the credential you need to create a Slack integration in Notify.
...
<Step title="Create the app">
    Click **Create App**. You'll be directed to the **Basic Application** for the Slack app which contains the credentials you need for configuring Slack chat inside Notify:
        - App ID
        - Client ID
        - Client Secret
...
These scopes are required for Notify to send messages, read channels and read users (for DMs and pickers). If you remove some of them, then certain features like channel or user selection might not work.
...
<Step title="Paste the Notify callback URL">
    Paste the Notify OAuth callback URL. Add the redirect URL that matches your Notify region:
...
## Configure Slack integration in Notify

Once your Slack app is set up, the next step is to configure the Slack Chat integration inside Notify.

<Steps>
  <Step title="Log in to the Notify dashboard">
    Open the [Notify Dashboard](https://dashboard.novu.co).
  </Step>
...
<Step title="Fill in credentials">
    ...
    - **Redirect URL (Optional)**: Enter the URL where you want users to be redirected to after they successfully connect their workspace. If there is no redirect URL, then Notify closes the tab immediately after the OAuth flow completes.
    ![Connect slack integration in Notify](/images/channels-and-providers/chat/slack/slack-integration.png)
  </Step>

<Step title="Create the integration">
    Click **Create Integration** to create the Slack integration. Once saved, Notify is able to:
...
To send messages to Slack, your users must first authorize Notify to access one of their Slack workspaces. This authorization happens through Slack's OAuth flow, which Notify generates and manages for you.

### Generate the OAuth URL

When a user clicks **Connect Slack** in your application, your backend should request a unique authorization URL from Notify.
...
Notify allows one connection per (integration + subscriber + context). To connect multiple workspaces, trigger separate OAuth flows with different combinations of those values.
...
<Step title="Redirect to Notify">
    Redirecting back to Notify's callback URL
  </Step>
</Steps>

After the user approves access, Notify handles the rest of the OAuth flow automatically.

<Steps>
  <Step title="Notify exchanges the code">
    Notify stores the Slack access token for the connected workspace.
  </Step>

<Step title="Notify creates a connection">
    Notify creates a Slack connection for that workspace, it is referenced by `connectionIdentifier` when creating endpoints.You can also provider a custom `connectionIdentifier` to the `generateChatOAuthUrl()` and then connection with such identifier will be created instead of randomly generated one.
  </Step>
</Steps>

<Note>
  **Alternative: handle OAuth yourself.** Instead of `generateChatOAuthUrl()`, you can run the Slack OAuth flow with your own app, use the access token with Slack's APIs directly, then register the workspace with Notify via [`channelConnections.create`](/api-reference/channel-connections/create-a-channel-connection) (or `POST /v1/channel-connections`):
...
<Step title="Notify creates webhook endpoint">
    Notify automatically creates a webhook endpoint for that subscriber using that URL.
  </Step>
</Steps>
...
When the workflow is triggered, Notify will:

<Steps>
  <Step title="Find Slack endpoints">
    Notify looks up Slack endpoints that match the subscriber ID and context.
  </Step>
...
<Step title="Use the workspace connection">
  ...
  <Step title="Deliver messages">
    Notify sends the notification to each configured Slack destination. By default, the Chat step body is sent as plain text. To send Block Kit formatting, see [Format messages with Block Kit](#format-messages-with-block-kit).
  </Step>
</Steps>

## Format messages with Block Kit

The Chat step body itself is plain text. To send richer Slack messages, layer a **Slack content override** on top of it. An override is a JSON object whose keys map to [Slack `chat.postMessage`](https://api.slack.com/methods/chat.postMessage) arguments, and Notify merges it into the request it sends to Slack.
...
<Steps>
  <Step title="Open the Chat step">
    In the [Notify Dashboard](https://dashboard.novu.co), open your workflow and select the Chat step that uses the Slack integration.
  </Step>
...
Override values may contain [Liquid](/platform/workflow/add-notification-content/personalize-content#apply-logic-with-liquidjs) templates, which Notify compiles at send time, so `{{payload.*}}` and `{{subscriber.*}}` resolve the same way they do in the step body:
...
`text` is Slack's primary content field. If your override does not set `text`, Notify fills it with the rendered Chat step body.
...
By default, Notify resolves `channel` and `token` from the subscriber's Slack endpoint and workspace connection. Notify does not set `as_user` — Slack deprecated it, and Notify posts as the installed bot.

If you pass `channel`, `token`, or `as_user` in step or trigger overrides (including `_passthrough.body`), Notify applies them as sent. You own the destination when you set these fields. Prefer addressing a different subscriber or topic when you want a different destination, rather than overriding routing keys.
...
Use `_passthrough` when you need a value to win unconditionally, or to reach a [Slack `chat.postMessage`](https://api.slack.com/methods/chat.postMessage) field Notify has not yet surfaced. Values in `_passthrough.body` are merged last and take priority over other override fields.
...
When a Slack override exists in more than one place, Notify merges them in this order, lowest priority first:
...
For [digest workflows](/platform/workflow/add-and-configure-steps/configure-action-steps/digest), Notify batches multiple triggers into one notification. The Chat step body can use [digest variables](/platform/workflow/add-notification-content/personalize-content#digest-variables) to summarize batched events, but trigger `overrides` are fixed when you call the API and are not re-evaluated against that aggregated data. To build Block Kit dynamically from a digest batch, use [Framework provider overrides](/framework/typescript/steps#providers-overrides-object) in a code-first workflow.

## Token rotation

Slack token rotation replaces long-lived bot tokens with short-lived access tokens and single-use refresh tokens. Notify stores the credentials returned during OAuth and refreshes the bot token automatically before it expires.

Apps created through Notify's **quick setup** (App Configuration Token) or the setup guide's manifest have token rotation enabled by default. Enabling token rotation on a Slack app is irreversible. Apps without token rotation continue to work with their long-lived bot tokens.
...
Notify converts the lifetime into an expiry time and refreshes the access token before it expires. Slack returns a new access token and refresh token after each successful refresh, and Notify stores the new pair for the next rotation.
...
Your Notify Slack integration must have the same **Client ID** and **Client Secret** that issued the token. Notify uses these credentials during refresh, and rejects the request if they are missing. If you omit `expiresAt`, Notify treats the token as expiring and refreshes it on the first send.
...
- The workspace was connected before Notify supported token rotation
...
<Card title="Build agents on Slack" icon="bot" href="/agents">
  Learn how Notify agents work, including managed and custom code agents.
</Card>
...
| **Workflow notification** | You trigger a workflow with a Chat step and Notify sends a one-way message to the subscriber's linked Slack destination. |
