---
title: "Webex Messaging Chat Integration with Notify"
sidebarTitle: "Webex Messaging"
description: "Connect Webex to Notify through OAuth and send workflow chat notifications to Webex rooms, threads, or people."
---

The Webex Messaging integration lets your application send Notify workflow notifications to Webex rooms and direct conversations. Your users authorize a Webex integration through OAuth, and Notify stores the resulting connection securely, refreshes its access token when needed, and routes each message to the room or person endpoint you register.
...
- A Notify environment and secret key.
...
<Steps>
  ...
  <Step title="Add the Notify redirect URI">
    Register the callback URI for the region where your Notify environment is hosted.
  ...
  <Step title="Select OAuth scopes">
    ...
    Webex adds `spark:kms` to registered integrations. Notify includes it when generating the authorization URL because Webex requires it for encrypted message content.
...
Notify requests these scopes by default:

| Scope | How Notify uses it |
| --- | --- |
...
## Configure Webex Messaging in Notify

<Steps>
  <Step title="Open the Notify dashboard">
    Sign in to the [Notify Dashboard](https://dashboard.novu.co).
  </Step>
  ...
  <Step title="Select Webex Messaging">
    ...
    ![Select Webex Messaging from the Notify Chat provider catalog](/images/channels-and-providers/chat/webex-messaging/integration-store.jpg)
  </Step>
  ...
  <Step title="Enter the credentials">
    ...
    - **Redirect URL** (optional): Where Notify sends the browser after OAuth succeeds. If omitted, Notify displays a success page.
    - **Base URL** (optional): Leave empty to use `https://webexapis.com/v1`.

    ![Configure Webex Messaging credentials in the Notify Integration Store](/images/channels-and-providers/chat/webex-messaging/configure-integration.jpg)
  </Step>

  <Step title="Create the Notify integration">
...
<Note>
  The optional **Redirect URL** in Notify is different from the Webex redirect URI. Webex must redirect to Notify's regional OAuth callback. The Notify field controls where the user goes after Notify finishes the callback.
</Note>
...
After authorization, Notify exchanges the code, reads the authorized person's Webex organization, and stores the access and refresh tokens on the connection. Notify refreshes the access token automatically before message delivery when it is close to expiring.

### Connect and link the current person in one flow

For a subscriber-scoped connection, set `autoLinkUser` to `true`. Notify creates the connection and a `webex_person` endpoint for the Webex person who completes OAuth.
...
The subscriber signs in to Webex through the returned URL. Notify reads their identity from `people/me` and creates a `webex_person` endpoint linked to the existing connection.

## Choose delivery destinations

Every Webex endpoint must reference a Webex Messaging connection through `connectionIdentifier`. If the connection has a Notify context, the endpoint must use the same context.
...
When the workflow runs, Notify resolves matching Webex endpoints, obtains a valid access token from the referenced connection, and posts the Chat step body to the Webex Messages API.
...
| --- | --- |
| OAuth fails with an invalid redirect URI | Confirm the Webex integration contains the Notify callback for the correct region. The value must match exactly. |
...
| Endpoint context mismatch | Use the same Notify `context` values on the endpoint and its referenced channel connection. |
