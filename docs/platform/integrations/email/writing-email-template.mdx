---
title: 'How to Write Email Templates in Notify'
sidebarTitle: 'Writing Email Template'
description: 'Create email templates in Notify using the block editor, custom HTML with LiquidJS, or the API. Personalize content with subscriber and payload variables.'
---

You can create email templates in Notify using the block editor, code editor, or API. Choose the block editor for visual layouts, the code editor for full HTML and LiquidJS control, or submit templates programmatically via the API.
...
![Notify code editor](/images/channels-and-providers/email/writing-email-template/code-editor.png)
...
You can also submit custom HTML email templates via the Notify API. This method supports workflows where you render templates externally and use Notify purely for delivery.

During the sending phase:

- Notify compiles your submitted HTML and LiquidJS template using the payload and subscriber context.
- The final rendered output is what gets delivered to recipients.
- LiquidJS is executed on Notify’s servers - there’s no need to pre-render the template yourself.

This approach works well if you manage your templates in your own systems and want Notify to act as the delivery engine.
...
- If your workflow defines a payload schema, then Notify uses that schema to generate preview data.
