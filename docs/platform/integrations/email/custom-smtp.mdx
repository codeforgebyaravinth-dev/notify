---
title: "Custom SMTP Email Integration with Notify"
sidebarTitle: "Custom SMTP"
description: "Connect a custom SMTP server to Notify to send transactional emails through workflows. Configure host, port, TLS, and credentials for any SMTP provider."
---

You can use a Custom SMTP provider like [Nodemailer](https://nodemailer.com/about/) to send transactional emails through your custom SMTP server to your customers using the Notify Platform with a single API.

## Getting Started

To use the Custom SMTP provider in the email channel, you will need to have your personal SMTP server configured and add `host`, `port`, `user`, and `password` to the Custom SMTP integration on the Notify platform.
...
## Creating a Custom SMTP integration with Notify

- Visit the [Integrations](https://dashboard.novu.co/integrations?utm_campaign=docs-customsmtp) page on Notify.
...
- You should now be able to send notifications using Custom SMTP in Notify.
