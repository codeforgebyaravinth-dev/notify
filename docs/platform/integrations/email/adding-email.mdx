---
title: "Adding Email Channel Email Integration with Notify"
sidebarTitle: "Adding Email Channel"
description: "Add an email channel provider in Notify to send transactional emails through workflows. Covers selecting a provider, storing credentials, and testing delivery."
---
...
<Steps>
  <Step title="Enable Email Channel in Notify Dashboard">

<Note>
  By default, the Email channel is enabled and configured with Notify's default provider. If it is
  disabled, notifications sent to this channel will not be processed.
</Note>

1. Go to the Notify Dashboard and click **"Integrations"** on the left sidebar
...
<Step title="Setup workflow with email channel in Notify Dashboard">
    
    1. Go to the Notify Dashboard and click **"Workflows"** on the left sidebar.
...
1. Go to the Notify Dashboard, navigate to the "Workflows" section, and locate your configured workflow.
    2. Click **"Test Workflow"** and provide sample data (e.g., user ID, email address).
    3. Verify the email delivery in the Notify Logs or your email provider dashboard.
