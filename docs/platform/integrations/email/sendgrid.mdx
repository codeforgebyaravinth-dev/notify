---
title: "SendGrid Email Integration with Notify"
sidebarTitle: 'SendGrid'
description: "Connect SendGrid to Notify to send transactional emails through workflows. Store your SendGrid API key and verified sender identity in the Notify dashboard."
---

You can use the [SendGrid](https://sendgrid.com/) provider to send transactional emails to your customers using the Notify Platform with a single API to create multi-channel experiences.

### Getting Started

To use the Sendgrid provider in the email channel, you will need to create a Sendgrid account and add your API key to the SendGrid integration on the Notify platform.
...
### SendGrid integration with Notify

- Visit the [Integrations store](https://dashboard.novu.co/integrations?utm_campaign=docs-sendgrid) on the Notify web dashboard.
...
- You should now be able to send notifications through SendGrid using Notify.

### Using SendGrid template

Notify has its own email editor for writing email template. To send pre-made template in SendGrid, providers overrides can be used to send template details. Make sure sendgrid `Api Key` has enough permission to read and process the template.
