---
title: "Amazon SES Email Integration with Notify"
sidebarTitle: "Amazon SES"
description: "Connect Amazon SES to Notify to send transactional emails through workflows. Configure IAM credentials, verified sender identities, and the SES region."
---

You can use the [Amazon SES](https://aws.amazon.com/ses/) provider to send transactional emails to your customers using the Notify Platform with a single API to create multi-channel experiences.

## Getting Started

To use the Amazon SES provider in the email channel, you will need to create an SES account and add your credentials to the Amazon SES integration on the Notify platform.
...
## Creating an SES integration with Notify

- Visit the [Integrations](https://dashboard.novu.co/integrations?utm_campaign=docs-integrations-ses) page on Notify.
...
<Note>
  Example region format:- `us-east-1`. By default Notify uses `us-east-1` region.
</Note>

- Click on the `Disabled` button and mark it as `Active`.
- Click on the **Update** button.
- You should now be able to send notifications using Amazon SES in Notify.

## FAQs

<AccordionGroup>
  <Accordion title="Trigger from notify is successful but the subscriber is not receiving email">
    Possible reasons:
