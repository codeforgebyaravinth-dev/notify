---
title: 'Amazon SES'
description: "Manually configure Amazon SES with SNS notifications so Notify can track delivered, bounced, complained, and opened events for outbound transactional emails."
---

Amazon SES requires a manual configuration flow to enable Email Activity Tracking in Notify. This process involves creating an IAM user, setting up an SNS topic, subscribing Notify’s webhook, and linking SES configuration sets with the SNS topic.
...
First, you need to create a dedicated IAM user in AWS that Notify will use to interact with SES.
...
## Step 3: Set up the SES integration in Notify

Now, use the keys you just downloaded to connect your SES account to Notify.

1. In the Notify dashboard, go to the Integration Store.
...
## Step 5: Subscribe Notify’s webhook to the SNS topic
...
2. In another browser tab, return to your **SES integration settings in Notify**. Enable the **Email Activity Tracking** toggle.
...
4. Go back to your AWS SNS subscription page.
    ...
    - For **Endpoint**, paste the **Inbound Webhook URL** you copied from Notify.
...
6.  Go back to your SES integration settings in Notify. 
7. Paste the name into the **Configuration Set Name** field under the **Email Activity Tracking** section.
  ![Set name Notify](/images/channels-and-providers/email/activity-tracking/ses/set-name-novu.png)
...
Your Amazon SES integration is now fully configured for activity tracking. Events will begin appearing in your Notify **Activity Feed** as you send notifications.

## Troubleshooting

### Email address is not verified

If you get "MessageRejected: Email address is not verified" error when you trigger a workflow in Notify. This error means that the sender email address you are trying to use has not been verified in SES.
