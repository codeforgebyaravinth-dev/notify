---
title: "Sinch Mailgun Email Integration with Notify"
sidebarTitle: "Sinch Mailgun"
description: "Connect Sinch Mailgun to Notify to send transactional emails through workflows. Configure your Mailgun API key, sending domain, and region for reliable email delivery."
---

You can use [Sinch Mailgun](https://www.mailgun.com/) to send transactional emails to your customers through Notify.

Mailgun is Sinch's email product. In the Notify Integrations Store it still appears as **Mailgun**. Use the same credentials from your Sinch Mailgun account.

## Getting Started

To use Sinch Mailgun in the email channel, create a Mailgun account and add your API key and domain name to the Mailgun integration on the Notify platform.
...
## Creating a Sinch Mailgun integration with Notify

- Visit the [Integrations](https://dashboard.novu.co/integrations?utm_campaign=docs-mailgun) page on Notify.
...
- You should now be able to send notifications using Sinch Mailgun in Notify.
