---
title: "Pushpad Push Integration with Notify"
sidebarTitle: 'Pushpad'
description: "Connect Pushpad to Notify to send web push notifications through workflows. Store your Pushpad project ID and auth token in the Notify dashboard for delivery."
---

This guide walks you through the entire process of configuring and using [Pushpad](https://pushpad.xyz) with Notify, from getting your credentials to sending your first notification.
...
## Configure Pushpad with Notify

Before you can send notifications, you need to connect your Pushpad project to Notify by getting your credentials and adding them to your integration settings.
...
<Step title="Record the Project ID">
    Click **Settings** on the project page and record the **Project ID**. You need it to connect to Notify.
...
<Step title="Copy the token">
    Copy the generated token. You need it to connect to Notify.
    ![Generate access token](/images/channels-and-providers/push/pushpad/access-token.png)
  </Step>
</Steps>

### Step 2: Connect Pushpad to Notify

Add these credentials to your Pushpad integration in the Notify dashboard.
...
<Step title="Paste credentials">
    ...
    ![Pushpad Integration in Notify](/images/channels-and-providers/push/pushpad/pushpad-integration.png)
  </Step>
...
This `uid` is the identifier Notify uses to target a specific browser. For example, if you use `pushpad('uid', 'user123')`, then `user123` is the ID you must register in Notify.
...
Now you're ready to send a push notification. [Create a workflow with a Push step](/platform/workflow/create-a-workflow) and then trigger it. Notify sends the notification to the `uid`s associated with the subscriber.
