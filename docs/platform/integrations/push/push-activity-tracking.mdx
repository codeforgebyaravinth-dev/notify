...
description: "Manually forward push notification delivery, open, and dismissal events from your mobile or web app to Notify for unified activity tracking across channels."
---

To enable activity tracking for Push channel notifications, Notify supports a manual integration approach for push notifications. Where your application captures and forwards push notification events to Notify. Once received, Notify processes and displays these events on the dashboard for a unified tracking experience.

## How it works

The process involves a four-step data flow from your subscriber's device to Notify's servers:
...
3.  **Backend forwards to Notify:** Your server receives this data and uses the Notify SDK to securely forward the event to Notify's API.
4.  **Event appears in Notify:** Notify processes the event and displays it in the **Activity Feed**, alongside events from your other channels.

## Step 1: Enable push activity tracking in Notify

Enable push activity tracking in your Notify dashboard and get the necessary credentials.
...
3. Enable the **Push Activity Tracking** toggle.
  ![Enable push activity tracking toggle in the Notify integration store](/images/channels-and-providers/push/activity-tracking/enable-activity-tracking.png)
...
- `eventId`: The unique identifier for the notification, which Notify includes in the push payload as `__nvMessageId`.
...
## Step 3: Forward events to Notify from your backend

Create an endpoint on your backend that receives the event data from your application and uses the Notify SDK to forward it to Notify.
...
const novuClient = new Notify({ secretKey: process.env.NOVU_SECRET_KEY });
...
novu_client = Notify(secret_key=os.getenv("NOVU_SECRET_KEY", ""))

@app.post("/api/notifications/events")
async def track_push_event(req_body: dict):
    with novu_client as notify:
...
$sdk = notify\Notify::builder()->setSecurity(getenv('NOVU_SECRET_KEY'))->build();
...
Once these steps are completed, your application will send push notification engagement data to Notify. This gives you a complete, unified view of your notification performance in the Activity Feed.