---
title: "OneSignal Push Integration with Notify"
sidebarTitle: 'OneSignal'
description: "Connect OneSignal to Notify to send push notifications to mobile and web through workflows. Store your OneSignal app ID and API key in the Notify dashboard."
---

This guide walks you through the entire process of configuring and using [OneSignal](https://onesignal.com/) with Notify, from getting your credentials to sending your first notification.
...
## Configure Onesignal with Notify

Before you can send push notifications via OneSignal from Notify, you need to connect your OneSignal credentials.
...
### Step 2: Connect Onesignal to Notify

Next, add these keys to your OneSignal integration in the Notify dashboard.
...
<Step title="Paste credentials">
    ...
    ![Onesignal Integration in Notify](/images/channels-and-providers/push/onesignal/onesignal-integration.png)
  </Step>
...
To target a OneSignal user from Notify, you must register their `player_id` as the `deviceToken` for their Notify subscriber profile.
...
By default, Notify uses `player_id` to send notifications, but you can select the `External ID` option in the OneSignal integration settings in Notify. If `External ID` option is selected, then `deviceTokens` stored in subscriber credentials for the OneSignal provider, are used as external user IDs.

By default, Notify uses player IDs to send notifications. If your OneSignal integration uses external user IDs, then you can switch this behavior in the OneSignal integration settings in Notify.
