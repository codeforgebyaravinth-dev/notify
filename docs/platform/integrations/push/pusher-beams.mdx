---
title: "Pusher Beams Push Integration with Notify"
sidebarTitle: "Pusher Beams"
description: "Connect Pusher Beams to Notify to send push notifications to iOS, Android, and web devices through workflows using your Pusher Beams instance ID and secret key."
---

[Pusher Beams](https://pusher.com/beams/) is a cross-platform push notification API service provided by Pusher.

This guide explains the process of configuring and using Pusher Beams with Notify, from getting your credentials to sending your first notification.

## How to configure Pusher Beams with Notify

Before you can send notifications, you must get your credentials from a Pusher Beams instance and add them to your Notify integration settings.
...
### Step 2: Connect Pusher Beams to Notify

Next, add these credentials to your Pusher Beams integration in the Notify dashboard.
...
<Step title="Paste credentials">
    ...
    ![Pusher Beams Integration in Notify](/images/channels-and-providers/push/pusher-beams/pusher-beams-integration.png)
  </Step>
...
## Using Pusher Beams with Notify
...
To target a Pusher Beams user from Notify, you must register this `userId` as the `deviceToken` for their Notify subscriber profile. You can retrieve this value using the [`getUserId()`](https://pusher.com/docs/beams/reference/web/#getuserid) method from the Pusher Beams SDK.
...
Now you're ready to send a push notification. [Create a workflow with a Push step](/platform/workflow/create-a-workflow) and trigger it. Notify sends the notification to the `userId`'s associated with the subscriber.
