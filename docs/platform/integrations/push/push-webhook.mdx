---
title: "Push Webhook Push Integration with Notify"
sidebarTitle: "Push Webhook"
description: "Connect a push webhook provider to Notify to deliver push notifications through your own HTTP endpoint or a custom push gateway using a webhook URL and headers."
---

This guide walks you through the entire process of configuring and using Push Webhook with Notify.

The Push Webhook provider is different from other push providers because it does not depend on a third-party service. Instead, Notify sends push notifications directly to a webhook URL that you control. This approach is ideal if you want full control over how notifications are processed, routed, or stored in your system.

## Configure Push Webhook with Notify

To configure Push Webhook, you must provide an endpoint URL for Notify to call and a secret HMAC key to verify the request's authenticity.

### Step 1: Get your webhook URL and secret key

Before connecting to Notify, you need two things:

* **Webhook URL**: This is your own API endpoint that will receive the `POST` request from Notify. For quick testing, you can use a service like [webhook.site](https://webhook.site/).
* **Secret HMAC Key:** This is a self-generated secret string. Notify will use it to encrypt the payload using the `HMAC SHA256` algorithm and send the hash in the `x-notify-signature` header. This lets you verify that the request is genuinely from Notify. See [how to generate a HMAC key](/platform/inbox/prepare-for-production#2-generate-hmac-hash-on-the-server-side).

<Note>
  Your webhook URL endpoint must be able to accept `POST` requests.
</Note>

### Step 2: Connect Push Webhook to Notify

Next, add these keys to your Push Webhook integration in the Notify dashboard:
...
<Step title="Open Integration Store">
    On the Notify dashboard, navigate to the **Integration Store**.
  </Step>
...
<Step title="Fill in integration fields">
    ...
    * **Secret HMAC Key:** The secret key used to sign webhook calls.
    ![Push Webhook Integration in Notify](/images/channels-and-providers/push/push-webhook/push-webhook-integration.png)
  </Step>
...
## Using Push Webhook with Notify
...
Now you're ready to send a push notification. [Create a workflow with a Push step](/platform/workflow/create-a-workflow) and trigger it. Notify sends the notification payload to the webhook URL that you configured.

The example below demonstrates a simple trigger using Notify’s SDK.
...
## Payload sent by Notify to webhook URL

When you trigger a workflow, Notify sends a `POST` request to your webhook URL with a JSON body similar to the one below.
...
If you provided a secret HMAC key during configuration, then Notify will include a `x-notify-signature` header in the request. You can use this header to verify that the request is from Notify and not a malicious third party.
...
async function acceptNovuPushWebHookRequest(request, response) {
  ...
  const hmacHashSentByNovu = request.headers['x-notify-signature'];
...
if (hmacHashSentByNovu === actualHashValue) {
    ...
    console.log('Request sent by Notify');
  } else {
