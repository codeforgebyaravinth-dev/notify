---
title: "Gupshup SMS Integration with Notify"
sidebarTitle: 'Gupshup'
description: "Connect Gupshup SMS to Notify to send SMS notifications through workflows. Store your Gupshup user ID, password, and source in the Notify dashboard for delivery."
---

You can use the [Gupshup](https://www.gupshup.io/) provider to send SMS messages to your customers using the Notify Platform with a single API to create multi-channel experiences.

## Getting Started

To use the Gupshup provider in the SMS channel, the first step is to create a Gupshup account and add the Account User Id and Password to the Gupshup integration on the Notify platform.
...
## Creating a gupshup integration with Notify

- Visit the [Integrations](https://dashboard.novu.co/integrations?utm_campaign=docs-sms-gupshup) page on Notify.
...
Now it is possible to send SMS notifications using Gupshup in Notify.

## Using Gupshup templateId

Notify has its own SMS editor for writing SMS template. To use premade Gupshup templates, providers overrides can be used.
