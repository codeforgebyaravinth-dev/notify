---
title: "SMS Webhook SMS Integration with Notify"
sidebarTitle: "SMS Webhook"
description: "Connect an SMS webhook provider to Notify to send SMS through your own HTTP API or a custom SMS provider endpoint using a webhook URL, headers, and JSON payload."
---

SMS Webhook lets you send SMS notifications through any HTTP API you control. Instead of a built-in provider SDK, Notify `POST`s each message to your **Base URL** with the subscriber phone number, message body, and sender ID.

Use this integration when your SMS gateway exposes a REST API that is not listed in Notify's provider catalog, or when you want Notify to call an internal service that handles delivery.

## Configure SMS Webhook in Notify
...
<Steps>
  ...
  <Step title="Open the Integration Store">
    In the Notify dashboard, go to **Integrations Store** and click **Connect Provider**.
  </Step>
  ...
  <Step title="Fill in integration fields">
    ...
    | --- | --- | --- |
    | **Base URL** | Yes | The URL Notify calls to send each SMS. |
    ...
    | **Auth URL** | If token auth | URL Notify calls to obtain the token. |
...
For each SMS, Notify sends a `POST` request to your **Base URL** with the configured API key headers.
...
Return a `2xx` JSON response. Notify reads the message ID and date using the **Id Path** and **Date Path** you configured.
...
If the date field is missing, Notify uses the current timestamp.

## Token authentication (optional)

When **Authenticate by token** is enabled:

1. Notify `POST`s to your **Auth URL** with the API key headers.
2. Notify reads the token from `data.<Authentication Token Key>` in the response.
3. Notify sends the SMS request to **Base URL** with that token in a header named after **Authentication Token Key**.
...
<Note>
  On Notify Cloud, **Base URL** and **Auth URL** must be publicly reachable HTTPS endpoints. Private or internal URLs are blocked by SSRF protection.
</Note>
