---
title: "Grafana Tool Integration with Notify"
sidebarTitle: 'Grafana'
description: "Send alerts to different Grafana IRM/OnCall stacks per subscriber from a single Notify workflow. Store each subscriber's incoming-webhook URL as a channel endpoint and create alert groups from a Tool step."
---

Grafana in Notify is routed **per subscriber**. Each subscriber brings their own Grafana IRM/OnCall incoming-webhook (Formatted Webhook) URL, so one workflow trigger can create alert groups on different Grafana stacks and escalation chains for different recipients. The integration itself never stores a webhook URL. It acts as the anchor that per-subscriber [channel endpoints](/api-reference/channel-endpoints/create-a-channel-endpoint) hang off of.
...
<Steps>
  ...
  <Step title="Copy the webhook URL">
    Grafana generates a unique webhook URL such as `https://<stack>.grafana.net/integrations/v1/formatted_webhook/<token>/`. Copy it. This is the URL Notify will store for the subscriber.
  </Step>
</Steps>
...
Register each subscriber's webhook URL as a **channel endpoint** on the Grafana integration. Notify encrypts the URL (and the optional `authToken`) at rest on `ChannelEndpoint.endpoint` and returns the wire shape `{ url, authToken? }` on reads.

<Warning>
  Registering an endpoint requires your Notify secret key. Always call the channel-endpoints API from your server, never from a browser or mobile client. A typical flow is: your app UI sends the webhook URL to **your** backend, and your backend forwards it to Notify.
</Warning>
...
Send the webhook URL as a `grafana_oncall_integration` endpoint. Set `createSubscriberIfMissing: true` so this call also provisions the Notify subscriber the first time an end user connects Grafana, with no separate identify call required.
...
Reads return the full wire shape (including the webhook URL), so you can display connection status. Mask it in your UI (the Notify dashboard shows only the last four characters).
...
<Steps>
  ...
  <Step title="Trigger the workflow">
    Send a trigger for a subscriber who has connected Grafana. Notify resolves that subscriber's webhook URL and creates an alert group on their Grafana stack.
  </Step>
</Steps>
...
The deterministic `alert_uid` means Notify retries for the same step group into the same Grafana alert instead of creating duplicates.

## What happens without an endpoint

If a workflow triggers for a subscriber who has **no** Grafana endpoint on the target integration, Notify marks the step as **skipped** in the Activity feed for that subscriber and does not create an alert. Other subscribers on the same trigger are unaffected. Each is routed to their own Grafana stack.
