---
title: "Opsgenie Tool Integration with Notify"
sidebarTitle: 'Opsgenie'
description: "Create Opsgenie alerts for different subscribers from a single Notify workflow. Store each subscriber's API integration key as a channel endpoint and raise alerts from a Tool step."
---

Opsgenie in Notify is routed **per subscriber**. Each subscriber brings their own Opsgenie API integration key (GenieKey), so one workflow trigger can raise alerts in different Opsgenie accounts and teams for different recipients. The integration itself never stores an API key. It acts as the anchor that per-subscriber [channel endpoints](/api-reference/channel-endpoints/create-a-channel-endpoint) hang off of.
...
- Access to the [Notify dashboard](https://dashboard.novu.co)
...
<Steps>
  ...
  <Step title="Copy the API key">
    Opsgenie generates a UUID-format **API Key** (for example `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`). Copy it. This is the GenieKey Notify will store for the subscriber.
  </Step>
</Steps>
...
## Add Opsgenie in Notify

<Steps>
  <Step title="Open the Integrations Store">
    In the Notify dashboard, click **Integrations Store** in the sidebar.
  </Step>
...
Register each subscriber's API key as a **channel endpoint** on the Opsgenie integration. Notify encrypts the API key at rest on `ChannelEndpoint.endpoint` and returns the wire shape `{ apiKey, region }` on reads (`region` stays plaintext).

<Warning>
  Registering an endpoint requires your Notify secret key. Always call the channel-endpoints API from your server, never from a browser or mobile client. A typical flow is: your app UI sends the API key to **your** backend, and your backend forwards it to Notify.
</Warning>
...
sequenceDiagram
  ...
  participant NovuAPI as Notify API
...
Send the API key as an `opsgenie_integration` endpoint. Set `createSubscriberIfMissing: true` so this call also provisions the Notify subscriber the first time an end user connects Opsgenie, with no separate identify call required.
...
| `createSubscriberIfMissing` | `boolean` | Optional. When `true`, Notify creates the subscriber on the fly if it does not exist yet. Existing subscribers are never modified. Defaults to `false`. |
...
Reads return the full wire shape (including the API key), so you can display connection status. Mask it in your UI (the Notify dashboard shows only the last four characters).
...
<Steps>
  ...
  <Step title="Write the alert message">
    The step content becomes the alert `message` in the Opsgenie Alert API request. Use dynamic placeholders such as `{{payload.orderNumber}}` or `{{subscriber.firstName}}`. Opsgenie caps `message` at 130 characters; Notify truncates longer content.
  </Step>

  <Step title="Trigger the workflow">
    Send a trigger for a subscriber who has connected Opsgenie. Notify resolves that subscriber's API key and creates an alert in their Opsgenie account. Opsgenie accepts the request asynchronously (`202` with a `requestId`).
  </Step>
</Steps>
...
| `alias` | Deterministic: `notify:{transactionId}:{subscriberId}:{stepId}`. | Explicit `alias` step override (up to 512 characters). |
...
The deterministic `alias` is Opsgenie's deduplication key: Notify retries for the same step will deduplicate into the same Opsgenie alert instead of creating duplicates.

## What happens without an endpoint

If a workflow triggers for a subscriber who has **no** Opsgenie endpoint on the target integration, Notify marks the step as **skipped** in the Activity feed for that subscriber and does not create an alert. Other subscribers on the same trigger are unaffected. Each is routed to their own Opsgenie account.
