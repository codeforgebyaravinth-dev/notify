---
title: "Tool Webhook Tool Integration with Notify"
...
- For **dynamic** routing: your Notify **secret key** to register endpoints from your server
...
<Steps>
  ...
  <Step title="Choose routing mode and defaults">
    ...
    | **Signing Secret** | Optional | Optional | HMAC secret for the `X-Notify-Signature` header (see [Verify signatures](#verify-x-notify-signature)). |
  </Step>
...
Notify sends `Content-Type: application/json`. The exact JSON body depends on whether you configured a **Body** template on the integration.
...
## Verify `X-Notify-Signature`

When you set a **Signing Secret** on the integration, Notify signs the **exact UTF-8 request body string** with HMAC SHA-256 and sends the hex digest in the `X-Notify-Signature` header.
...
app.post('/novu-tool-webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  const signature = req.headers['x-notify-signature'] as string | undefined;
...
<Warning>
  Re-stringifying a parsed JSON object often produces a different byte sequence than Notify signed. Always verify against the raw request body.
</Warning>

Your endpoint should return any `2xx` response. Notify treats non-2xx responses as delivery failures.

## Dynamic mode: register subscriber endpoints

In **dynamic** routing, each subscriber can have **multiple** `tool_webhook` endpoints on the same integration. Notify delivers the Tool step once per endpoint — useful when a subscriber wants parallel notifications to several systems.

<Warning>
  Registering endpoints requires your Notify secret key. Call the channel-endpoints API from your server, never from a browser or mobile client.
</Warning>
...
Register a subscriber destination with type `tool_webhook`. Set `createSubscriberIfMissing: true` to provision the Notify subscriber on first connect.
...
await novu.channelEndpoints.create({
  ...
  endpoint: {
    url: 'https://customer.example.com/hooks/notify',
...
| `createSubscriberIfMissing` | `boolean` | Optional. When `true`, Notify creates the subscriber if missing. Defaults to `false`. |
| `endpoint.url` | `string` | HTTPS or HTTP URL Notify calls for this endpoint. Encrypted at rest. |
...
<Steps>
  ...
  <Step title="Trigger the workflow">
    Trigger the workflow for a subscriber. In static mode Notify calls the integration URL. In dynamic mode Notify calls every `tool_webhook` endpoint registered for that subscriber.
  </Step>
</Steps>
...
If **Routing Mode** is **Dynamic** and a subscriber has **no** `tool_webhook` endpoints on the integration, Notify marks the Tool step as **skipped** for that subscriber in the Activity feed. Other subscribers on the same trigger are unaffected.
