---
title: 'Common errors'
description: "Diagnose and resolve common Notify workflow errors including validation failures, provider issues, rate limits, and step execution problems."
---

The Activity Feed page in the Notify dashboard displays all the events that occur when a workflow is triggered. Each triggered workflow includes a log for every step, showing whether a notification failed and why. Below are some common errors that you might encounter.
...
## Notify's provider limit has been reached

Each environment in Notify is provisioned with demo providers for email and SMS channels. These demo providers are limited to 300 emails and 20 SMS per month.
