...
description: "Enable and safely render raw HTML in the notification subject and body of the Notify Inbox component for rich text formatting and inline styling."
---

By default, Notify sanitizes the `subject` and `body` of all in-app step notifications to ensure that they are safe to display. This process prevents security risks by removing suspicious HTML tags (like `<script>`) and escaping HTML entities (like `&`, `<`, `>`).
...
Enabling HTML formatting is a two-step process: you must first disable sanitization in your Notify workflow and then use the appropriate render prop in your application to display the HTML.

## Step 1: Disable content sanitization in your workflow

In your Notify workflow:
1. navigate to the **In-App editor**.
2. In the editor, find and turn on the **Disable content sanitization** toggle.

![Disable content sanitization in the Notify in-app step editor](/images/inbox/disable-content-sanitization.png)

This setting instructs the Notify API to send the raw, unescaped HTML content for both the subject and body to your application.
