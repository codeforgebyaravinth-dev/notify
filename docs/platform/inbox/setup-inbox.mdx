...
description: "Integrate the Notify Inbox component into your application to display real-time in-app notifications for your subscribers with drop-in React or JS components."
---
...
<Note>
  **`@notify/react` requires React 18 or later.** The package supports React `^18.0.0` and React 19 (`^19.0.0`). React 17 and below are not supported. When rendering the Inbox UI, you also need `react-dom` at the same version.
</Note>
...
To display real-time notifications for your subscribers, connect the Inbox component to your Notify environment using your `applicationIdentifier` and a `subscriberId`. You can create or manage subscribers from the [Notify Dashboard](https://dashboard.novu.co/subscribers).
...
If your Notify account is in the EU region, then include the `backendUrl` and `socketUrl` props to connect to EU-specific API endpoints:
