...
description: "Migrate from the legacy @notify/notification-center package to the new @notify/react Inbox with a step-by-step guide covering setup and props."
---

<Note>
  - Use [@notify/react](/platform/sdks/react) Inbox React component and
  [@notify/js](/platform/sdks/javascript) headless package with new dashboard workflows and [@notify/framework](/platform/workflow)-based workflows.

   - With legacy dashboard based workflows, use
  [@notify/notification-center](https://v0.x-docs.novu.co/notification-center/client/react/get-started) package.
</Note>

The `@notify/react` package introduces a more flexible and customizable way to display notifications in your application. This guide outlines the key differences between the `@notify/notification-center` package and the new `@notify/react` package. 

Follow the steps below to migrate your application to the latest Inbox version.

## Why you should upgrade to `@notify/react`

- **Customization**: The `@notify/react` package provides more customization options for the appearance and behavior of the notification components.
...
- **Compatibility with the `@notify/framework`**: The `@notify/react` package is designed to work seamlessly with the `@notify/framework` package for creating and managing notifications.

## Breaking Changes

The `@notify/react` package introduces several breaking changes compared to the `@notify/notification-center` package. Here are the key differences:
...
To begin, install the `@notify/react` package.
...
### Legacy implementation with `@notify/notification-center`
...
import {
  ...
  NotificationBell,
} from '@notify/notification-center';

function Notify() {
  return (
    <NotifyProvider subscriber="YOUR_SUBSCRIBER_ID" applicationIdentifier="YOUR_APP_ID">
      <PopoverNotificationCenter>
...
### Current implementation with `@notify/react`
...
The @notify/react package introduces a flexible way to display notifications as a list without the default bell icon. Use the `Inbox` and `Notifications` components to achieve this functionality.

### Legacy implementation with `@notify/notification-center`

```tsx
import { NotifyProvider, NotificationCenter } from '@notify/notification-center';
...
```tsx
import { NotifyProvider, PopoverNotificationCenter } from '@notify/notification-center';
...
Handle user interactions with notifications effectively using the action handlers provided by `@notify/react`.
    ...
    const handleOnActionClick = async (
      ...
      if (templateIdentifier === 'friend-request') {
        if (btnType === 'primary') {
          ...
          /** And then update Notify that this action has been taken, so the user won't see the button again **/
          updateAction({
...
In the legacy implementation, you could set a notification’s avatar icon by enabling the Add an avatar option in the workflow UI. Notify would then use the avatar field of the actor subscriber as the icon.
...
### Current implementation with `@notify/react` and Radix UI as an example

```tsx
import React from 'react';
import * as RadixPopover from '@radix-ui/react-popover';
import { Inbox, Bell, Notifications } from '@notify/react';
...
If you still have questions or need further assistance, please reach out to us at [support@notify.co](mailto:support@notify.co).