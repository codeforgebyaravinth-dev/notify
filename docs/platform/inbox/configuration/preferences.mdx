---
title: 'Preferences'
description: "Configure and customize subscriber notification preferences directly inside your application using the built-in Preferences view of the Notify Inbox."
---

The Preferences interface in the Inbox component lets subscribers customize how they receive notifications. By default, Notify renders a preferences icon inside the Inbox component, giving users access to global and workflow-specific settings directly from the UI.

<Note>
  A workflow marked as "critical" on the Notify dashboard cannot be disabled by the user and will not appear in their preferences UI. You can override this behavior by setting the `criticality` field in the `preferencesFilter` prop.
</Note>
...
<Note>
  The name given to a workflow in the Notify dashboard is the name your subscribers will see in their preferences UI.
</Note>
...
```tsx
import { Inbox, WorkflowCriticalityEnum } from '@notify/react';
...
```tsx
import { Inbox, PreferenceLevel } from '@notify/react';
...
You don’t have to display the Preferences UI inside the default Inbox component. Notify provides two main ways to build a custom preference experience for your subscribers.
