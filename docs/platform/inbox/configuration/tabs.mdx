---
title: 'Tabs'
description: "Configure and filter tabs in the Notify Inbox component to group notifications by workflow, tag, or custom filter criteria for your users."
---
...
[Tags](/platform/workflow/configure-workflow#tags) can be assigned to any workflow in the Notify workflow editor. These tags can then be used in the filter object to group notifications from one or more workflows under the same tab.

For example, assign a **promotions** tag to marketing workflows or **security** and **alert** tags to security workflows to group their notifications under dedicated tabs. To show all the notifications in a tab, pass an empty array to the tags property. If multiple tags are provided, Notify evaluates them using `OR` logic, meaning a notification is included if it contains at least one of the specified tags.
