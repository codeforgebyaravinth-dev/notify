...
description: 'Style and theme the prebuilt Notify Inbox component with appearance keys, custom CSS variables, and light or dark mode overrides to match your brand.'
---


The Notify Inbox component is designed to be fully themeable and adaptable to your application’s visual language. All theming options are exposed through the `appearance` prop, which allows you to apply custom styles at different levels of control from predefined themes to component-level overrides.
...
Notify currently provides a built-in dark theme, which you can import from `@notify/react/themes`.
...
On mobile and smaller devices, use the `inbox__popoverContent` element and apply a custom CSS class to it. Specify CSS media queries on this class and add the class in a global CSS file so that it takes effect. In the example below, media queries are applied to the `notify-inbox-popover-content` class in a global CSS file.
...
const ResponsiveInbox = () => {
  return (
    <Inbox
      ...
      appearance={{
        elements: {
          inbox__popoverContent: "notify-inbox-popover-content",
        },
      }}
    />
  );
