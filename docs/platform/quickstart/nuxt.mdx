---
title: 'Nuxt In-App Notifications Quickstart - Notify Inbox'
description: "Create a Notify account and integrate the Notify Inbox component into your Nuxt application to display real-time in-app notifications for your subscribers."
sidebarTitle: 'Nuxt'
---


Add real-time in-app notifications to Nuxt using the Notify Inbox component and the `@notify/js` SDK.

<Note>
  This guide uses the `@notify/js` JavaScript SDK to build the Inbox component in Nuxt. Notify currently does not support a native Nuxt Inbox component.
</Note>

<Prompt description="Add Notify Inbox to my Nuxt app" icon="sparkles" actions={["copy", "cursor"]}>
# Add Notify Inbox to Nuxt App

Install `@notify/js`. Mount the Inbox in your header, navbar, or sidebar. Wrap the Inbox in `<ClientOnly>` because Nuxt renders with SSR by default and the Inbox is client-only.
...
<Steps>
  ...
  ### Create a Nuxt application
    ...
    ```package-install
    npx nuxi@latest init notify-inbox-nuxt
    ```
  </Step>
  ...
  Create the `app/components/NotifyInbox.vue` file to add the Inbox component passing <Tooltip tip="The application identifier is a unique identifier for your application. You can find it in the Notify Dashboard under the API keys page.">applicationIdentifier</Tooltip> and <Tooltip tip="The subscriber ID is the unique identifier for the user in your application, typically the user's id in your database.">subscriberId</Tooltip>:

  ```vue title="app/components/NotifyInbox.vue"
  <template>
    <div ref="novuInbox"></div>
  </template>
  ...
  <Note>
  Explore the full `nuxt-inbox` example in the [Notify examples repository](https://github.com/jainpawan21/notify-inbox-nuxt).
</Note>
  ...
  Import and use the `NotifyInbox` component in `app/app.vue`. Wrap it with `<ClientOnly>` so the Inbox renders only in the browser. Nuxt enables SSR by default, and the Inbox does not support server-side rendering.
  ...
  <script setup lang="ts">
  import NotifyInbox from './components/NotifyInbox.vue';
  </script>
