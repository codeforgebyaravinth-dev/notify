---
title: 'React In-App Notifications Quickstart - Notify Inbox'
description: "Integrate the Notify Inbox component into your React application, add routing with React Router, and start showing real-time in-app notifications to your users."
sidebarTitle: 'React'
---


Add real-time in-app notifications to React in under 10 minutes using the Notify Inbox component.

<Prompt description="Add Notify Inbox to my React app" icon="sparkles" actions={["copy", "cursor"]}>
# Add Notify Inbox to React App
...
<Steps>
  ...
  ## Create a React app using Vite
    ...
    ```package-install
    npm create vite@latest notify-inbox-react -- --template react-ts
    cd notify-inbox-react
    npm install
    npm run dev
    ```
  ...
  In the `src` directory, create a `components/notify-inbox.tsx` file and use the [`<Inbox />`](/platform/inbox) component, passing <Tooltip tip="The application identifier is a unique identifier for your application. You can find it in the Notify Dashboard under the API keys page.">applicationIdentifier</Tooltip> and <Tooltip tip="The subscriber ID is the unique identifier for the user in your application, typically the user's id in your database.">subscriberId</Tooltip>:

  <Note>
  Explore the full `react-inbox` example in the [Notify examples repository](https://github.com/novuhq/novu).
</Note>
  ...
  ## Set up React Router and import the Inbox component
    Now you can set up React Router and add the `NotifyInbox` component to your app layout:
