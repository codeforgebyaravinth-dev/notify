---
title: 'Webhooks'
description: "Configure webhook endpoints in Notify to receive real-time event notifications for workflow updates, message delivery, and subscriber activity."
---

Webhooks makes it possible for your external systems to receive real-time notifications when specific events occur within your Notify account. They are POST requests sent to a pre-determined endpoint that you configure.
...
You provide Notify with a URL (endpoint), and we send a JSON payload to that URL whenever an event happens, such as a workflow update or message delivery.
...
To start listening to messages, you must register your endpoint in the Notify dashboard.

1. Go to the **[Webhooks](https://dashboard.novu.co/webhooks)** page in the Notify dashboard.
...
When adding or editing an endpoint, you can utilize the advanced configuration option to fine-tune how Notify communicates with your server.

- **Rate limiting (Throttling)**: You can configure rate limits to effectively throttle the webhooks, ensuring Notify only sends a specific number of requests per second.
...
You must verify that the requests hitting your endpoint are actually from Notify and not a malicious actor.

Notify signs webhook requests so you can verify that payloads were sent by Notify. Official libraries are available for verifying signatures.
...
When an event cannot be delivered, Notify retries using exponential backoff. Each message is attempted based on the following schedule, where each period is started following the failure of the preceding attempt:
...
When your endpoint is unavailable, Notify keeps retrying according to the retry schedule. When all attempts fail for 5 days consecutively, the endpoint will be disabled and you have to manually re-enable it.
...
Endpoints can be manually enabled or disabled at any time. Notify also automatically disables an endpoint when all deliveries fail for 5 consecutive days.

To enable or disable an endpoint:
1. Go to the webhook page of the Notify dashboard.
...
When Notify receives a response with a 2xx status code, it is interpreted as a successful delivery even regardless of the response payload. Make sure to use the right response status codes so Notify knows when messages are supposed to succeed or fail.

### Responses timing out

Notify considers all messages that fails to send a response within 15 seconds as a failed message. 
