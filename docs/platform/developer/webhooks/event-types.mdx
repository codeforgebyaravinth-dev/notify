---
title: 'Event types'
description: "Reference of every event type Notify emits through webhooks, including message, subscriber, workflow, and delivery events with example payload structures."
---

Notify supports the following webhook event types:

- **Email events**: Events when Notify receives inbound mail on a verified domain with a webhook route.
...
- `email.received`: Triggered when Notify receives an inbound email that matches an [Inbound Email](/platform/inbound-email/overview) **Webhook** route on a verified domain. The payload includes normalized mail content (from, to, subject, text, html, headers, attachments, threading fields) plus domain and route metadata.
...
```json
{
  "object": {
    ...
    "mail": {
      ...
      "inReplyTo": "<original-message-id@notify.co>",
      "references": ["<thread-root@notify.co>"],
      "headers": {},
      "attachments": [],
...
- `message.failed`: This webhook is triggered when Notify tries to send the message to the delivery provider and it got failed. The payload contains the details of the event.
...
- `message.sent`: This webhook is triggered when Notify sends the message to the delivery provider. The payload contains the details of the event.
