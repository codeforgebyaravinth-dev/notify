---
title: 'Webhooks'
description: "Learn how Notify webhooks work, including event types, payload schemas, HMAC signature verification, retries, and how to configure endpoints in the dashboard."
---
...
For example, if you receive webhooks from Notify, you can structure your URL like: `https://www.example.com/novu/webhook/`.
...
Notify webhooks allow you to receive notifications when specific events occur in your Notify account. These events include workflow updates, subscriber changes, message delivery status updates, and inbound email received on your verified domains.
...
- **email.received**: Triggered when Notify receives inbound mail matching an Inbound Email webhook route. See [Event types - Email events](/platform/developer/webhooks/event-types#email-events) for the payload schema and [Inbound Email](/platform/inbound-email/overview) for setup.
...
- **messages.sent**: Triggered when Notify sends the message to the delivery provider.
...
- **messages.failed**: Triggered when Notify tried to send the message to the delivery provider and it got failed.
...
Webhook signatures let you verify that webhook messages are actually sent by Notify and not a malicious actor.

Verifying webhook signatures helps ensure payloads were sent by Notify and have not been tampered with. Here is an example using JavaScript:
...
<AccordionGroup>
  ...
  <Accordion title="What happens if my endpoint is unavailable?">
    If your endpoint is unavailable, Notify will retry sending the webhook according to the retry schedule. If all attempts fail for 5 days, the endpoint will be disabled and you'll need to manually re-enable it.
  </Accordion>
