...
description: "Create reusable, environment-scoped variables in the Notify dashboard and reference them inside workflow steps to avoid hardcoding secrets and URLs."
---

Environment variables in Notify are named values you define once in the dashboard and reuse across workflows. Each [Notify environment](/platform/developer/environments) can hold its own value for the same variable, so Development and Production (and any custom environments) stay isolated without duplicating workflow logic.
...
If a variable has no value in an environment where a workflow references it, Notify can surface a warning on the Environment variables page (for example when a value is missing in one environment but set in others) and in workflow step preview or execution context, so you can add or correct the value before relying on it in production.
...
You can use these references anywhere Notify accepts Liquid syntax, including:
