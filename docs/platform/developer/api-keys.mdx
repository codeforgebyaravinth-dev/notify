---
title: 'API keys'
description: 'Find your Notify application identifier, secret key, and API hostnames, and rotate secret keys without downtime using rolling rotation.'
---

Notify provides a set of keys and hostnames that your application uses to authenticate requests, send events to the Notify API, and to interact with the [`<Inbox />`](/platform/inbox).

Your API keys are environment specific and allow Notify to tie all request to a particular Notify environment.

## Finding your API keys

Follow these steps to find your environment-specific API keys:
1. Login to the Notify dashboard.
...
The Application Identifier is a unique identifier for your application within Notify. It is considered a public key and is safe to expose in client-side code.
...
The following hostnames are used to connect to the Notify cloud platform.

| **Endpoint**           | **URL**               | **Description**                                                                                                                          |
| ---------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **API Hostname**       | `https://api.novu.co` | The domain that your application sends API requests to. It identifies the Notify API server.                                               |
