---
title: Agent Toolkit
description: Expose Notify notification workflows as tools for LLM agents with the @notify/agent-toolkit package. Works with OpenAI, LangChain, and Vercel AI SDK.
---

## What is the Agent Toolkit?

The `@notify/agent-toolkit` package lets you expose [Notify](https://novu.co) notification workflows as tools for LLM agents. It works with **OpenAI**, **LangChain**, and **Vercel AI SDK**.

The toolkit automatically discovers your Notify workflows and converts them into strongly-typed tools that an LLM can invoke, letting your AI agent send notifications, manage subscriber preferences, and trigger any workflow you've built in Notify.

<Note>
  The Agent Toolkit is a code-level SDK for embedding Notify tools inside your own AI agents. For giving AI coding assistants (Claude Code, Cursor, Copilot) context about Notify, see [Agent Skills](/platform/build-with-ai/skills). For connecting AI tools to Notify via the Model Context Protocol, see [MCP](/platform/build-with-ai/mcp).
</Note>

## Installation

```bash
npm install @notify/agent-toolkit
```
...
| --- | --- | --- |
| OpenAI | `openai >= 4.0.0` | `@notify/agent-toolkit/openai` |
| LangChain | `@langchain/core >= 0.2.0` | `@notify/agent-toolkit/langchain` |
| Vercel AI SDK | `ai >= 6.0.0` | `@notify/agent-toolkit/ai-sdk` |

## Quick Start

```typescript
import { createNovuAgentToolkit } from '@notify/agent-toolkit/openai';
...
| --- | --- | --- |
| `secretKey` | Yes | Your Notify API secret key from [dashboard.novu.co/settings/api-keys](https://dashboard.novu.co/settings/api-keys). |
| `subscriberId` | Yes | Default subscriber ID used when triggering workflows. |
| `backendUrl` | No | Custom Notify API URL (defaults to Notify Cloud). |
...
<CodeGroup>
```typescript title="Basic setup"
import { createNovuAgentToolkit } from '@notify/agent-toolkit/langchain';
...
<CodeGroup>
```typescript title="Basic setup"
import { createNovuAgentToolkit } from '@notify/agent-toolkit/ai-sdk';
...
Triggers any Notify workflow by its identifier. Use this as a generic entry point to send notifications.
...
On initialization the toolkit fetches your Notify workflows and creates a dedicated tool for each one. These tools are named `trigger_<workflow_id>` (with hyphens replaced by underscores) and include the workflow's payload schema so the LLM knows exactly what data to provide.
...
- **Customer support agents** - Let an AI agent send order confirmations, shipping updates, or support ticket notifications through Notify workflows.
...
The `@notify/agent-toolkit` package is part of the [Notify monorepo](https://github.com/novuhq/novu/tree/next/packages/agent-toolkit) on the `next` branch.
...
| --- | --- |
| `@notify/agent-toolkit` | Core `NovuToolkit` class, `NovuTool` factory, and types. |
| `@notify/agent-toolkit/openai` | OpenAI adapter with `createNovuAgentToolkit`. |
| `@notify/agent-toolkit/langchain` | LangChain adapter with `createNovuAgentToolkit`. |
| `@notify/agent-toolkit/ai-sdk` | Vercel AI SDK adapter with `createNovuAgentToolkit`. |
| `@notify/agent-toolkit/core` | Core classes and types (for building custom adapters). |
