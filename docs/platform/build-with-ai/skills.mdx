---
title: Agent Skills
description: "Use Notify Agent Skills to help AI coding assistants build multi-channel notification systems in your codebase, with guided prompts and reusable patterns."
---


## What are Agent Skills?

Agent Skills are an open standard that gives AI agents (Claude Code, Cursor, Copilot, etc.) the context they need to work with specific tools and platforms. Notify's skills provide AI agents with everything required to build multi-channel notification systems - triggering workflows, managing subscribers, integrating the in-app inbox, and configuring preferences.
...
Install the Notify skills into your project using the `skills` CLI:
...
Copy or open these prompts in Cursor after installing Notify skills with `npx skills add novuhq/skills`. Replace `YOUR_*` placeholders with values from your Notify dashboard.

### Core skills

<Prompt description="Send a welcome notification" icon="bot" actions={["copy", "cursor"]}>
Send a welcome notification to subscriber-123 using the welcome-email workflow. Use @notify/api with NOVU_SECRET_KEY from environment variables.
...
Follow the notify-trigger-notification skill. Do not create documentation files - implement the trigger in my existing codebase.
</Prompt>

<Prompt description="Create a subscriber" icon="bot" actions={["copy", "cursor"]}>
Create a subscriber with subscriberId user-456, email jane@example.com, and firstName Jane in my Notify account using @notify/api.

Follow the notify-manage-subscribers skill. Store NOVU_SECRET_KEY in environment variables.
</Prompt>

<Prompt description="Add a bell icon to my app" icon="bot" actions={["copy", "cursor"]}>
Add a notification bell (Notify Inbox) to my Next.js app navbar. Install @notify/nextjs, use applicationIdentifier YOUR_APPLICATION_IDENTIFIER and subscriberId from my auth system.

Follow the notify-inbox-integration skill. Place the Inbox inline in existing UI - no new pages or wrappers.
...
Follow the notify-manage-preferences skill using @notify/api with NOVU_SECRET_KEY from environment variables.
</Prompt>

### Additional skills

These skills are available in the [Notify documentation MCP](/platform/build-with-ai/mcp) and Mintlify-hosted skill packages:

<Prompt description="Set up a code-first workflow" icon="bot" actions={["copy", "cursor"]}>
Set up a code-first Notify workflow using @notify/framework in my Next.js app. Create a welcome-email workflow with an email step and show me how to trigger it.

Follow the notify-framework-integration skill. Use NOVU_SECRET_KEY from environment variables.
...
Follow the notify-design-workflow skill. Ask clarifying questions about my channels and subscriber preferences before proposing the workflow structure.
</Prompt>
...
Follow the notify-dashboard-workflows skill. Use Liquid variables for personalization.
</Prompt>
...
| --- | --- | --- |
| `@notify/api` | Server | Trigger notifications, manage subscribers/topics/workflows via REST |
| `@notify/react` | Client | React Inbox component, Notifications, Preferences, Bell |
| `@notify/nextjs` | Client | Next.js-optimized Inbox integration |
| `@notify/react-native` | Client | React Native hooks-based Inbox integration |
| `@notify/js` | Client | Vanilla JavaScript client for non-React apps |

## Skill Reference

### Trigger Notification

Send notifications by triggering Notify workflows. Supports single, bulk, broadcast, and topic-based delivery.
...
Notify uses a two-level preference system: **workflow defaults** and **subscriber overrides** (set by end users).

**Workflow-level defaults**: Workflow level defaults can be configured in the Notify Dashboard. To do so, click on the workflow you want to configure and then click on the `Manage Preferences` option.
...
For additional help, check the [Notify documentation](https://docs.novu.co) or contact us at [support@notify.co](mailto:support@notify.co).