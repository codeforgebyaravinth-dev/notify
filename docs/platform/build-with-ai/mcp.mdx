...
description: Connect Cursor, Claude, ChatGPT, and other AI tools to the Notify MCP server to manage notifications, workflows, and subscribers using natural language.
---

The Notify MCP Server exposes Notify's notification infrastructure as tools that AI assistants can discover and use in real time. Connect your AI tool to manage subscribers, workflows, integrations, and delivery activity from natural-language prompts.

<Note>
  Notify also hosts a separate **documentation MCP server** at [docs.novu.co/mcp](https://docs.novu.co/mcp) that lets AI tools search and read this documentation site. The Notify MCP Server described on this page manages your Notify account - triggers, workflows, subscribers, and more.
</Note>

## About the Notify MCP server
...
Connecting with **OAuth** is recommended - you don't copy or paste a key; your client signs you in and picks up your organization automatically. If your client doesn't support OAuth, or you're on self-hosted Notify, use an [API key](#api-key-fallback) instead.
...
Save the file. Cursor will prompt you to sign in to Notify the first time it calls a tool.
...
```bash
claude mcp add --transport http notify https://mcp.novu.co/
```
...
Add Notify as a custom connector using the MCP URL for your region, and choose **OAuth** as the connection mechanism. ChatGPT will prompt you to sign in to Notify.
  ...
  "mcpServers": {
    "notify": {
      ...
      "args": [
        ...
        "https://mcp.novu.co/",
        "--header",
        "Authorization:Bearer your-notify-api-key"
      ]
    }
  }
}
...
```bash
export NOVU_API_KEY="your-notify-api-key"
```
...
Use a Notify API key for clients without OAuth support, `mcp-remote` / stdio setups, autonomous agents, and **all self-hosted Notify** deployments. Copy your secret key from the [Notify Dashboard](https://dashboard.novu.co/settings/api-keys) and send it as a bearer token:
...
List my Notify environments, then show notifications from Production (not Development).
</Prompt>

## Example prompts

After connecting the Notify MCP server, copy or open these prompts in Cursor to verify the connection and perform common tasks.

<Note>
  Replace placeholder values (for example, subscriber IDs and workflow identifiers) with values from your Notify dashboard.
</Note>

### Verify connection

<Prompt description="Test your Notify MCP connection" icon="bot" actions={["copy", "cursor"]}>
Run `whoami` to verify I'm authenticated and show my Notify region.
</Prompt>

<Prompt description="List your notification workflows" icon="bot" actions={["copy", "cursor"]}>
Show me all my notification workflows in Notify.
</Prompt>

### Subscriber management

<Prompt description="Find a subscriber by email" icon="bot" actions={["copy", "cursor"]}>
Find subscriber user@example.com in my Notify account and show their profile details.
</Prompt>

<Prompt description="Create a new subscriber" icon="bot" actions={["copy", "cursor"]}>
Create a new subscriber with subscriberId user-456, email jane@example.com, and firstName Jane in my Notify account.
</Prompt>
...
You can connect both the Notify MCP server and the [Notify documentation MCP server](https://docs.novu.co/mcp) at the same time. Each serves a different purpose:

| Server | URL | Purpose |
| --- | --- | --- |
| Notify MCP | `https://mcp.novu.co/` (US) / `https://eu.mcp.novu.co/` (EU) | Manage your Notify account - workflows, subscribers, triggers |
| Notify Docs MCP | `https://docs.novu.co/mcp` | Search and read Notify documentation |

Connected MCP servers don't consume context until the AI calls a tool. Be specific in your prompts so the assistant uses the most relevant server - for example, "trigger my welcome workflow" uses the Notify MCP, while "how do I configure digest steps?" uses the Docs MCP.
...
<AccordionGroup>
  ...
  <Accordion title="Self-hosted Notify">
...
- Self-hosted Notify always uses API keys; OAuth is available for Notify Cloud only.
- Use caution when combining the Notify MCP server with other MCP servers or untrusted data in the same session, and review tool calls before approving actions that modify your data.

## Related topics

<Columns cols={2}>
  <Card title="Agent Skills" href="/platform/build-with-ai/skills">
    Give AI coding assistants implementation guidance for Notify SDKs and workflows.
  </Card>
  <Card title="Agent Toolkit" href="/platform/build-with-ai/agent-toolkit">
    Embed Notify tools inside your own AI agents with the `@notify/agent-toolkit` package.
  </Card>
  <Card title="Notify Docs MCP" href="https://docs.novu.co/mcp">
