...
description: "Build a fully custom subscription and preference management interface with Notify using headless hooks that expose data, mutations, and state."
---

Notify `@notify/react` package provides Subscription hooks that let you build your own custom subscription experiences from scratch.

These hooks allow you to list, create, update, and delete subscriptions while leveraging Notify's state management.

<Note>
  Hooks are available in [`@notify/nextjs`](/platform/sdks/react), [`@notify/react-native`](/platform/sdks/react-native), [`@notify/react`](/platform/sdks/react). For a complete guide including server API and JSON Logic conditions, see [Manage topic subscriptions](/platform/subscription/manage-topic-subscriptions). For hook reference, see the [API reference documentation](/platform/sdks/react/hooks/subscription).
</Note>
...
```tsx
import { useSubscriptions } from '@notify/react';
...
```tsx
import { useSubscription } from '@notify/react';
...
```tsx
import { useCreateSubscription } from '@notify/react';
...
```tsx
import { useUpdateSubscription } from '@notify/react';
...
```tsx
import { useRemoveSubscription } from '@notify/react';
