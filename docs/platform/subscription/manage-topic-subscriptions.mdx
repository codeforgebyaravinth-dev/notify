...
description: 'Create and update topic subscriptions using the REST API, @notify/js, and @notify/react, including JSON Logic conditions evaluated at trigger time.'
---
...
<Note>
  Conditions are evaluated at **trigger time**, not when the subscription is created. A subscription is always stored; Notify decides at delivery whether the condition matches the incoming event data.
</Note>
...
Notify uses **exact-match** Context filtering for subscriptions, the same rules as the [Inbox](/platform/inbox/configuration/inbox-with-context):
...
<Note>
  When you omit Context on a subscription or trigger, Notify treats it as the **default** (no Context) scope, not as a wildcard that matches all Contexts.
</Note>
...
- **`@notify/js` / `@notify/react`**: pass `context` when initializing the Notify client or `<NotifyProvider>`. The session inherits that Context for all subscription API calls.

**At trigger time**, pass the same Context on the workflow trigger. Notify resolves topic members whose stored Context matches the trigger Context, then evaluates each matching subscription's JSON Logic condition.

If Notify auto-generates a subscription `identifier` and Context is present, the identifier includes a `:ctx_` suffix so subscriptions in different Contexts remain unique.
...
| [Inbox API](#inbox-api) | Subscriber JWT (via `@notify/js` / `@notify/react`) | Subscriber self-service: follow buttons, preference centers, custom UIs |
| [`@notify/js`](#novujs) | Subscriber JWT | Headless JavaScript/TypeScript apps |
| [`@notify/react`](#novureact) | Subscriber JWT | React/Next.js apps with hooks or `<Subscription />` components |
...
The Inbox API is the subscriber-facing surface that powers `@notify/js` and `@notify/react`. It uses a subscriber session token (created from your `applicationIdentifier` and `subscriberId`) instead of an API key.
...
<Card title="@notify/js reference" icon="book" href="/platform/sdks/javascript#subscriptions">
...
If you want a ready-made subscribe/preferences UI, use `<Subscription />`, `<SubscriptionButton />`, and `<SubscriptionPreferences />` inside `<NotifyProvider>`:

```tsx
import { NotifyProvider, Subscription, SubscriptionButton, SubscriptionPreferences } from '@notify/react';
...
See the [quickstart](/platform/subscription/quickstart) for setup steps. For Context on `<NotifyProvider>`, see [Inbox with Context](/platform/inbox/configuration/inbox-with-context).
...
<Steps>
  <Step title="Create a subscription with a condition">
    Subscribe a user to a topic and attach a JSON Logic rule to a workflow preference (server API, `@notify/js`, or `@notify/react`).
  </Step>
...
```
  ...
  <Step title="Notify matches Context, then evaluates conditions">
    Notify first selects topic subscriptions whose stored Context exactly matches the trigger Context (including the default/no-Context case). For each matching subscription, Notify evaluates the JSON Logic condition against the trigger **payload**, **subscriber** profile, optional **actor**, and resolved **context**. Subscriptions that pass receive the notification; others are skipped.
  </Step>
</Steps>
...
Notify Cloud bills by [workflow runs](/platform/account/billing#what-is-a-workflow-run): one execution of a workflow for one subscriber. Usage is counted when Notify creates a **workflow run** for that subscriber, not when you call the trigger API.

Topic subscription filtering happens **before** that point, during topic fan-out:

1. Notify looks up subscriptions on the topic (including [Context](/platform/concepts/contexts) exact-match filtering).
2. For each candidate subscription, Notify evaluates JSON Logic conditions against the trigger **payload**, **subscriber** profile, optional **actor**, and resolved **context**.
