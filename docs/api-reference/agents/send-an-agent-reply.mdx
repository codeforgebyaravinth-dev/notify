...
Send a reply into an existing agent conversation from server-side code. Use this when you are not running `@notify/framework` on the bridge (for example with the Python, Go, PHP, .NET, or Java SDKs), or when another backend service needs to post into a live conversation.
