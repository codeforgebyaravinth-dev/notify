...
description: "Authenticate REST API requests to Notify with your secret key, manage environment credentials, rotate keys, and follow security best practices."
---

The Notify REST API uses API key authentication. Every server-side request must include your environment's secret key in the `Authorization` header.
...
Each Notify environment has two credentials. Use the right one for the context:
...
The application identifier is safe to use in client-side code. It identifies your application within Notify but does not authorize API requests on its own.

## Find your API keys

1. Log in to the [Notify Dashboard](https://dashboard.novu.co).
...
Use the base URL that matches your Notify Cloud region:
...
<Warning>
  Your account and its API keys are bound to the region you chose at signup. A US secret key will not authenticate against `https://eu.api.novu.co` (and vice versa) - you cannot switch regions by swapping the base URL. To change region, contact [sales@notify.co](mailto:sales@notify.co); see [Data Residency](/platform/additional-resources/security#data-residency) for details.
</Warning>

<Note>
  Self-hosted deployments use your own API hostname. See [Self Hosting Notify](/community/self-hosting-notify/overview) for configuration details.
</Note>
