---
title: 'Idempotency'
description: "Use idempotency keys with the Notify API to safely retry POST and PATCH requests without triggering duplicate workflow runs or duplicate side effects."
---

Idempotency is a crucial feature for building reliable integrations with Notify's API. It allows you to safely retry requests without worrying about duplicate operations, which is especially important for critical actions like triggering workflows.
...
<Note>
  Idempotency feature is currently not enabled for all organizations. Please contact us at support@notify.co to enable it for your organization.
</Note>
...
1. Notify checks if a request with the same key has been processed before
2. If it's a new key, the request is processed normally and the response is cached
3. If the key was seen before, Notify returns the cached response without reprocessing the request
...
When making idempotent requests, Notify includes helpful headers in the response:
