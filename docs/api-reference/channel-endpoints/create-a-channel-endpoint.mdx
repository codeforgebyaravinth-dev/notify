...
Use type `tool_webhook` with a **Tool Webhook** integration configured for **Dynamic** routing. A subscriber may have **multiple** `tool_webhook` endpoints on the same integration — Notify delivers the Tool step once per endpoint.
