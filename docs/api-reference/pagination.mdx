---
title: 'Pagination'
description: 'Page through large Notify API list responses using cursor-based pagination with limit, after, and before query parameters and response metadata.'
---

List endpoints return results in pages so you can retrieve large collections efficiently. Notify uses **cursor-based pagination** as the standard for list endpoints. Pass an opaque cursor to walk forward or backward through results without skipping or duplicating items as your data changes.
