---
title: 'Rate Limiting'
description: "Learn Notify API rate limits by tier, read the RateLimit response headers, and handle 429 responses with exponential backoff and idempotency keys."
---
...
The following limits apply to each category of the Notify system. Each category has an independent bucket of request tokens to consume from.
...
When integrating with Notify API, it’s important to consider the rate limiting HTTP headers included in the response. These headers help you manage your API usage and avoid hitting rate limits.
